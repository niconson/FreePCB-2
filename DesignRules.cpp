...
void DRErrorList::HighLight( DRError * dre )
{
	...
	dl_element * dl1 = NULL; 
	dl_element * dl2 = NULL;
	if( dre->id1.type == ID_PART || dre->id1.type == ID_PART_LINES )
	{
		...
		if( part )
		{
			...
			if( dre->id1.st == ID_TXT )
				if( dre->id1.ii >= 0 && dre->id1.ii < part->m_outline_stroke.GetSize() )
					dl1 = part->m_outline_stroke[dre->id1.ii];
			if( dre->id1.st == ID_REF_TXT )
				dl1 = part->dl_ref_el;
			if( dre->id1.st == ID_VALUE_TXT )
				dl1 = part->dl_value_el;
		}
	}
	if( dre->id2.type == ID_PART || dre->id2.type == ID_PART_LINES )
	{
		...
		if( part )
		{
			...
			if( dre->id2.st == ID_TXT )
				if( dre->id2.ii >= 0 && dre->id2.ii < part->m_outline_stroke.GetSize() )
					dl2 = part->m_outline_stroke[dre->id2.ii];
			if( dre->id2.st == ID_REF_TXT )
				dl2 = part->dl_ref_el;
			if( dre->id2.st == ID_VALUE_TXT )
				dl2 = part->dl_value_el;
		}
	}
		...
		if( net1 )
		{
			...
			if( dre->id1.st == ID_CONNECT )
				if ( dre->id1.i < net1->nconnects )
					if( dre->id1.sst == ID_SEG )
					{
						if ( dre->id1.ii < net1->connect[dre->id1.i].nsegs )
							dl1 = net1->connect[dre->id1.i].seg[dre->id1.ii].dl_el; 
					}
					else if ( dre->id1.ii <= net1->connect[dre->id1.i].nsegs )
						dl1 = net1->connect[dre->id1.i].vtx[dre->id1.ii].dl_el; 
		}
	}
		...
		if( net2 )
		{
			...
			if( dre->id2.st == ID_CONNECT )
				if ( dre->id2.i < net2->nconnects )
					if( dre->id2.sst == ID_SEG )
					{
						if ( dre->id2.ii < net2->connect[dre->id2.i].nsegs )
							dl2 = net2->connect[dre->id2.i].seg[dre->id2.ii].dl_el; 
					}
					else if ( dre->id2.ii <= net2->connect[dre->id2.i].nsegs )
						dl2 = net2->connect[dre->id2.i].vtx[dre->id2.ii].dl_el; 
		}
	}
		...
		for( pos = list.GetHeadPosition(); pos != NULL; )
		{
			...
			if( dre->dl_el )
			{
				if( dre->dl_el->gtype != DL_HOLLOW_CIRC)
				{
					...
					int x = (r->right + r->left)/2;
					int y = (r->top + r->bottom)/2;
					r->left =	x - dre->dl_el->el_w*50;
					r->right =  x + dre->dl_el->el_w*50;
					r->bottom =	y - dre->dl_el->el_w*50;
					r->top =	y + dre->dl_el->el_w*50;
					dre->dl_el->el_w = 1;
				}
			}
		}

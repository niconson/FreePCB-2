...
#define TRANSPARENT_ZERO_W				LAY_DRC_ERROR
...
class dl_element
{
	...
	friend class CBoardOutlineImage;
...
public:
	...
	~CBoardOutlineImage();
	// add backgnd images
	void Load( CString * image, BOOL ToBottom );
...
private:
	...
	RECT m_rect;
	int m_invert;
	dl_element board_outline_left;
	dl_element board_outline_right;
};
...
public:
	...
	int GetPDFLayer( int order, int page, int * mirror_layer );
	...
	void MoveCanvas( CDC * dDC, int dx, int dy );
	int CancelHighLight( BOOL IncludeStatic=FALSE, BOOL IncludeMerge=FALSE );
	...
	int Intersection( dl_element * el1, dl_element * el2 );
};

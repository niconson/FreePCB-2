...
void CDlgAddMaskCutout::DoDataExchange(CDataExchange* pDX)
{
	...
	if( !pDX->m_bSaveAndValidate )
	{
		...
		m_combo_layer.InsertString( 1, "SCRIBING (V-cut)" );
...
BEGIN_MESSAGE_MAP(CDlgAddMaskCutout, CDialog)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlgAddMaskCutout::OnCbnSelchangeCombo1)
END_MESSAGE_MAP()
...
void CDlgAddMaskCutout::GetCurSel()
{
	...
	{
		m_radio_none.EnableWindow(0);
		m_radio_edge.EnableWindow(0);
		m_radio_full.EnableWindow(0);
		m_radio_none.SetCheck(0);
		m_radio_edge.SetCheck(0);
		m_radio_full.SetCheck(1);
	}
	...
	{
		m_radio_none.EnableWindow(1);
		m_radio_edge.EnableWindow(1);
		m_radio_full.EnableWindow(1);
	}
}

void CDlgAddMaskCutout::SetCurSel()
{
	if( m_layer == LAY_BOARD_OUTLINE )
...
void CDlgAddMaskCutout::OnCbnSelchangeCombo1()
{

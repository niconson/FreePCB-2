...
static CString last_ref_des = "";
static CString last_footprint = "";
static CString last_package = "";
static CString last_value = "";
...
void CDlgAddPart::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK2, m_via);
	if( m_n_list_locked && !m_new_part )
	...
	if( pDX->m_bSaveAndValidate )
	{
		...
		m_package = m_package.Trim();

		// outgoing
		// test for valid ref des
		CString ref_des_str, value;
		m_edit_ref_des.GetWindowText( ref_des_str );
		m_edit_value.GetWindowText( value );
		ref_des_str = ref_des_str.Trim();
		value = value.Trim();
		if( !m_multiple )
		{
		...
		if( !m_multiple || (m_multiple_mask & MSK_FOOTPRINT) )
		{
			...
			foot_str = foot_str.Trim();
			if( foot_str.GetLength() == 0 )
				if( m_ip >= 0 )
			...
			if( ref_des_str.Left(3) == "VIA" )
			{
				(*m_pl)[m_ip].ref_size = 0;
				(*m_pl)[m_ip].ref_width = 0;
				(*m_pl)[m_ip].value_vis = 0;
			}
		}
		...
		//if( m_shape.m_package.GetLength() == 0 )
		//	m_shape.m_package = m_shape.m_name;
	}
	else
	{
		// incoming
...
BEGIN_MESSAGE_MAP(CDlgAddPart, CDialog)
	...
	ON_BN_CLICKED(IDC_CHECK2, &CDlgAddPart::OnBnClickedCheck2)
	ON_CBN_DROPDOWN(IDC_EDIT_VALUE, &CDlgAddPart::OnCbnDropdownEditValue)
	ON_CBN_SELCHANGE(IDC_EDIT_VALUE, &CDlgAddPart::OnCbnSelchangeEditValue)
//	ON_CBN_SELENDOK(IDC_EDIT_VALUE, &CDlgAddPart::OnCbnSelendokEditValue)
//	ON_CBN_EDITUPDATE(IDC_EDIT_VALUE, &CDlgAddPart::OnCbnEditupdateEditValue)
//	ON_CBN_EDITCHANGE(IDC_EDIT_VALUE, &CDlgAddPart::OnCbnEditchangeEditValue)
END_MESSAGE_MAP()
...
BOOL CDlgAddPart::OnInitDialog()
{
				...
				if( num > 0 )
				{
					...
					while( !done )
					{
						...
						if( done )
						{
							...
							m_edit_value.SetWindowText( last_value );
						}
					}
				}
			}
	...
	else
	{
		...
		if( m_n_list_locked > 1 )
		{
			part_tree.EnableWindow(0);
			m_edit_package.EnableWindow(0);
			m_edit_footprint.EnableWindow(0);
			m_edit_search.EnableWindow(0);
			///m_edit_value.EnableWindow(0);
			
			m_edit_x.EnableWindow(0);
			m_edit_y.EnableWindow(0);
			m_list_side.EnableWindow(0);
			m_combo_angle.EnableWindow(0);
		}
...
void CDlgAddPart::OnBnClickedCheck2()
{
	static CString mem_ref="";
	if( m_via.GetCheck() )
	{
		CString s;
		m_edit_ref_des.GetWindowTextA( s );
		mem_ref = s;
		if( s.Left(3) != "VIA" )
		{
			int maxnum = 0;
			for( int i=0; i<m_pl->GetSize(); i++ )
			{
				if( (*m_pl)[i].deleted )
					continue;
				CString pref, suff;
				int num = ParseRef( &m_pl->GetAt(i).ref_des, &pref, &suff );
				if( pref.Compare("VIA") == 0 )
					maxnum = max( num, maxnum );
			}
		
			s.Format("VIA%d", maxnum+1 );
			m_edit_ref_des.SetWindowTextA( s );
		}
	}
	else
	{
		m_edit_ref_des.SetWindowTextA( mem_ref );
	}
}


void CDlgAddPart::OnCbnDropdownEditValue()
{
	// TODO: ???????? ???? ??? ??????????? ???????????
	if( m_edit_value.GetCount() == 0 )
	{
		CString s = theApp.m_Doc->m_app_dir + "\\components.txt";
		CStdioFile f;
		int ok = f.Open( s, CFile::modeRead, NULL );
		if(ok)
		{
			while( f.ReadString(s) )
				m_edit_value.AddString(s);
			f.Close();
		}
	}
}
...
void CDlgAddPart::OnCbnSelchangeEditValue()
{
	// TODO: ???????? ???? ??? ??????????? ???????????
	int item = m_edit_value.GetCurSel();
	if(item < 0 )
		return;
	CString s, s1, s2, s3, gs, pref, suff;
	m_edit_value.GetLBText(item, s);
	int fn = s.Find("]");
	int fd = s.Find("@");
	if(fn > 0 && fd > 0)
	{
		s1 = s.Left(fn).Right(fn-1);
		s2 = s.Right(s.GetLength()-fn-2);
		s2 = s2.Left(fd-fn-2);
		s3 = s.Right(s.GetLength()-fd-1);

		m_edit_ref_des.GetWindowTextA(gs);
		fn = ParseRef( &gs, &pref, &suff);
		if(fn == 0)
			fn = 1;
		s.Format("%s%d", s1, fn);
		s1 = s;
		if( gs.Left(3) != "VIA" )
			m_edit_ref_des.SetWindowTextA(s1);
		if( s2.GetLength() )
		{
			s = "";
			if( item > 0 )
				m_edit_value.GetLBText(item-1, s);
			if( s.Compare(s2) )
			{
				m_edit_value.InsertString(item,s2);
				m_edit_value.SetCurSel(item);
			}
			else
				m_edit_value.SetCurSel(item-1);
		}
		m_edit_footprint.SetWindowTextA("");
		m_edit_package.SetWindowTextA("");
		m_edit_search.SetWindowText( s3 );
		int p = part_tree.GetVisibleCount();  //LPNMTREEVIEW
		for( HTREEITEM item = part_tree.GetFirstVisibleItem();
			 item != NULL;
			 item = part_tree.GetNextVisibleItem(item) )
		{
			s = part_tree.GetItemText(item);
			if( s.Compare(s3) == 0 )
				part_tree.SelectItem(item);
		}
	}
}

...
void CDlgCAD::OnBnClickedGo()
{
	...
	if (m_n_x > 1 || m_n_y > 1)
	{
		...
		{
			CString errStr;
			errStr.Format(G_LANGUAGE == 0 ? "Enter the size of the technological fields for panelization" : "Введите размер технологических полей для панелизации");
		...
		{
			CString errStr;
			errStr.Format(G_LANGUAGE == 0 ? "Enter the size of the holes for panelization" : "Введите размер отверстий для панелизации");
			AfxMessageBox(errStr, MB_OK);
			return;
		}
		...
		{
			CString errStr;
			errStr.Format(G_LANGUAGE == 0 ? "Technological field Y cannot be zero if there is a signature in the panelization" : "Технологическое поле Y не может быть нулевым если присутствует подпись в панелизации");
			AfxMessageBox(errStr, MB_OK);
			return;
		}
	}
	...
	for( int i=0; i<27; i++ )
	{
		...
		if( layer )
		{
			...
			else
			{
				...
				line.Format("G04 ########################################################*\n");
				f.WriteString(line);
				line.Format("G04 # GENERATED BY SOFTWARE INFORMATION:                   #*\n");
				f.WriteString(line);
				line.Format("G04 # CAD System : FreePCB MFC Application / PlatForm      #*\n");
				f.WriteString(line);
				line.Format("G04 # CAD Version: %5.3f                                   #*\n", m_version);
				f.WriteString(line);
				line.Format("G04 # CAD Vendor : niconson.com                            #*\n");
				f.WriteString(line);
				line.Format("G04 # CAD Website: https://niconson.com/freepcb2           #*\n");
				f.WriteString(line);
				line.Format("G04 # CAD Repos:   https://github.com/niconson             #*\n");
				f.WriteString(line);
				line.Format("G04 ########################################################*\n");
				f.WriteString(line);

...
void CDlgNetlist::DoDataExchange(CDataExchange* pDX)
{
	...
	if( m_nl_comp )
	{
		...
		if( m_nl_comp > 1 )
		{
			m_button_nl_comp.EnableWindow(0);
			m_button_nl_width.EnableWindow(0);
		}
	}
}
...
//	ON_NOTIFY(LVN_ITEMCHANGED, IDC_LIST_NET, OnLvnItemchangedListNet)
	...
	ON_BN_CLICKED(IDC_CHECK_NL_COMP, OnBnClicked_nl_comp)
...
void CDlgNetlist::OnNMClickListNet(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	else if( n_sel == 1 )
	{
		...
		if( m_nl_comp == 0 )
		{
			m_button_edit.EnableWindow(1);
			m_button_delete_single.EnableWindow(1);	
		}
		else if( m_nl_comp < 2 )
			m_button_nl_width.EnableWindow(1);	
	}
	else if( n_sel > 1 )
	{
		m_button_edit.EnableWindow(0);
		...
		if( m_nl_comp < 2 )
			...
			m_button_nl_width.EnableWindow(0);		
		if( m_nl_comp == 0 )
			m_button_delete.EnableWindow(1);
		else
			m_button_delete.EnableWindow(0);
	}
	if( pResult )
...
void CDlgNetlist::OnBnClicked_nl_comp()
{
	m_nl_comp = m_button_nl_comp.GetCheck();
	m_button_add.EnableWindow(!m_nl_comp);
}
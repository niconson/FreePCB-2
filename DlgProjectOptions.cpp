...
//	
CListCtrl * m_list_comp;
int CALLBACK W_Compare( LPARAM lp1, LPARAM lp2, LPARAM type )
{
	if( type )
		return 0;
	else if( lp1 == 0 )
	...
	/*if( type != 0 )
		return 0;
	CString s;
	s = m_list_comp->GetItemText( lp1, 0 );
	float f1 = my_atof(&s);
	s = m_list_comp->GetItemText( lp2, 0 );
	float f2 = my_atof(&s);
	if( f1 == f2 )
		return 0;
	else if( f1 > f2 )
...
void CDlgProjectOptions::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK2, m_check_show_M);
	...
	if( pDX->m_bSaveAndValidate )
	{
		...
		else
		{
			...
			//
			m_bShow_merges = m_check_show_M.GetCheck();
...
BOOL CDlgProjectOptions::OnInitDialog()
{
	...
	m_check_show_M.SetCheck(m_bShow_merges);
	//
	return TRUE;
}
		...
		if( my_atof(&str) > _2540 )
		{
			...
			if( dlg_m_via_w >= 0 )
			{
				...
				if( dlg_m_via_hole_w >= 0 )
				{
					...
					CString wc = dlg.m_width;
					m_list_menu.SetItemData( ic, (LPARAM)my_atof(&wc) );
					...
					m_list_menu.SortItems( ::W_Compare, 0 );
					return;
				}
			}
		}
...
void CDlgProjectOptions::OnBnClickedButtonEdit()
{
	...
	else
	{
		...
		if( ret == IDOK )
		{
			...
			CString wc = dlg.m_width;
			m_list_menu.SetItemData( i_sel, (LPARAM)my_atof(&wc) );

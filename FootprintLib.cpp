...
void CFootLibFolder::IndexAllLibs( CString * full_path, CDlgLog * dlg_log )
{
	...
	{
		CStdioFile r;
		if( r.Open( theApp.m_Doc->m_app_dir+"\\default_cds.cfg", CFile::modeRead, NULL ) )
		{
			CString instr, key;
			while( r.ReadString(instr) )
			{
				instr = instr.Trim();
				CArray<CString> arr;
				ParseKeyString( &instr, &key, &arr );
				if( key == "full_library_folder" && arr.GetSize() )
				{
					m_full_path_to_folder = arr[0];
					break;
				}
			}
			r.Close();
...
CFootLibFolder * CFootLibFolderMap::GetFolder( CString * full_path, CDlgLog * log )
{
	...
	str.MakeLower();
	BOOL B = m_folder_map.Lookup( str, ptr );
	if( !B || ptr == NULL )
	{

...
ON_COMMAND(ID_FP_OPENSCAD, OnPolylineOpenScad)
ON_COMMAND(ID_FP_SET_ORIGIN, OnPolylineSetOrigin)
...
ON_COMMAND(ID_FP_FIRST_VTX, OnPolylineFirstCorner)
...
ON_COMMAND(ID_GEN_OPENSCAD, GenerateOpenscadFile)
ON_COMMAND(ID_FILE_GENERATE_3D, GenerateOpenscadFile)
...
ON_UPDATE_COMMAND_UI_RANGE(ID_ADD_GRAPHICLINE, IDC_VALUE_TO_TEXT, OnInvalidate)
END_MESSAGE_MAP()
/////////////////////////////////////////////////////////////////////////////
...
void CFootprintView::InitializeView()
{
	...
	EnableUndo( FALSE );
	EnableRedo( FALSE );
	Invalidate( FALSE );//InitializeView
	...
	for( int i=m_fp.m_outline_poly.GetSize()-1; i>=0; i-- )
		for( int ii=m_fp.m_outline_poly[i].GetNumCorners()-1; ii>=0; ii-- )
			if( m_fp.m_outline_poly[i].GetSel(ii) )
				m_fp.m_outline_poly[i].MoveCorner( ii, m_fp.m_outline_poly[i].GetX(ii)+dx, m_fp.m_outline_poly[i].GetY(ii)+dy, FALSE );
	// pads
...
void CFootprintView::MirrorGroup()
{
	...
	for( CText * it=m_fp.m_tl->GetFirstText(); it; it=m_fp.m_tl->GetNextText(&i) )
		if( it->m_selected )
		{
			...
			{
				int inc = 1;
				if( it->m_layer == LAY_FP_TOP_COPPER || it->m_layer == LAY_FP_BOTTOM_COPPER )
					inc = 2;
				if( Get_fp_Side( it->m_layer ) == 0 )
					it->m_layer += inc;
				else if( Get_fp_Side( it->m_layer ) == 1 )
					it->m_layer -= inc;
			}
			m_fp.m_tl->MoveText( it, -gx, gy, -it->m_angle, (it->m_mirror?0:1), it->m_bNegative, it->m_layer );
			if( it->m_angle <= 0 )
				it->m_angle +=360.0;
		}
	m_fp.Draw( m_fp.m_dlist, m_Doc->m_smfontutil );
		...
		for( int ic=m_fp.m_outline_poly[i].GetNumContours()-1; ic>=0; ic-- )
		{
			...
			if( n_sel >= sz )
			{
				if( n_sel > 1 || sz > 1 )
				{
					...
					{
						m_fp.m_outline_poly[i].RemoveContour(ic, 0);
					}
				}
			}
			else if( n_sel )
...
void CFootprintView::SelectItemsInRect( CRect r )
{
	...
	// sel/unsel
	int sel = 1;
	if( m_last_mouse_point.x < r.CenterPoint().x )
		sel = 0;
	RECT unnorm_r = r;
	r.NormalizeRect();
	// lines
			...
			if( InRange(x, r.left, r.right) )
				if( InRange(y, r.top, r.bottom) )
					if( m_dlist->m_vis[lay] )
					{
						...
						m_sel_id.Set( ID_PART_LINES, ID_OUTLINE, i, ID_CORNER, ii );
						vtx_sel++;
					}
		}
	}
			...
			if( InRange(pr.right, r.left, r.right) )
				if( InRange(pr.top, r.top, r.bottom) )
					if( InRange(pr.bottom, r.top, r.bottom) )
						if( (m_dlist->m_vis[LAY_FP_TOP_COPPER] && m_fp.m_padstack[i].top.shape && m_fp.m_padstack[i].top.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_BOTTOM_COPPER] && m_fp.m_padstack[i].bottom.shape && m_fp.m_padstack[i].bottom.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_INNER_COPPER] && m_fp.m_padstack[i].inner.shape && m_fp.m_padstack[i].inner.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_TOP_MASK] && m_fp.m_padstack[i].top_mask.shape && m_fp.m_padstack[i].top_mask.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_TOP_PASTE] && m_fp.m_padstack[i].top_paste.shape && m_fp.m_padstack[i].top_paste.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_BOTTOM_MASK] && m_fp.m_padstack[i].bottom_mask.shape && m_fp.m_padstack[i].bottom_mask.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_BOTTOM_PASTE] && m_fp.m_padstack[i].bottom_paste.shape && m_fp.m_padstack[i].bottom_paste.shape != PAD_DEFAULT) ||
							(m_dlist->m_vis[LAY_FP_PAD_THRU] && m_fp.m_padstack[i].hole_size && m_fp.m_padstack[i].hole_size) )
						{
							vtx_sel = 0;
	...
	for( CText * it=m_fp.m_tl->GetFirstText(); it; it=m_fp.m_tl->GetNextText(&i) )
	{
		if( m_dlist->m_vis[it->m_layer] )
			if( InRange(it->m_x, r.left, r.right) )
				...
				{
					CRect r2;
					m_fp.m_tl->GetTextRectOnPCB( it, &r2 );
					if( RectsIntersection( unnorm_r, r2 ) == 1 )
					{
						vtx_sel = 0;
						it->m_selected = sel;
					}
	...
	HighlightGroup();
	if( vtx_sel == 1 && m_cursor_mode != CUR_FP_GROUP_SELECTED )
		SetCursorMode( CUR_FP_POLY_CORNER_SELECTED );
	else
		SetCursorMode( CUR_FP_GROUP_SELECTED );
	Invalidate( FALSE );//SelectItemsInRect
}
...
void CFootprintView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	else if( point.x > m_left_pane_w && point.y < m_client_r.bottom - m_bottom_pane_h ) 
	{
		...
		else if( m_cursor_mode == CUR_FP_DRAG_GROUP )
		{
			...
			m_dlist->StopDragging();
			m_dx = m_last_cursor_point.x - m_start_point.x;
			m_dy = m_last_cursor_point.y - m_start_point.y;
			m_dragging_new_item = FALSE;
			MoveGroup(m_dx,m_dy);
			SetCursorMode( CUR_FP_GROUP_SELECTED );
			HighlightGroup();
		...
		else if( m_cursor_mode == CUR_FP_ADD_POLY || m_cursor_mode == CUR_FP_ADD_PCB_HOLE )
		{
		...
		else if( m_cursor_mode == CUR_FP_DRAG_POLY )
		{
				...
				&& p.y == m_fp.m_outline_poly[m_sel_id.i].GetY(0) )
			{
				...
				m_fp.m_outline_poly[m_sel_id.i].Close( m_polyline_style, 0 );
		...
		else if( m_cursor_mode == CUR_FP_DRAG_CENTROID )
		{
			...
			// moving 3D
			m_fp.Move3dObject(&m_fp.m_openscad_code, -(p.x - m_fp.m_centroid_x), -(p.y - m_fp.m_centroid_y), m_Doc->m_units);
			m_fp.Move3dObject(&m_fp.m_openscad_hole, -(p.x - m_fp.m_centroid_x), -(p.y - m_fp.m_centroid_y), m_Doc->m_units);
			//
			m_fp.Undraw();
...
void CFootprintView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	else if( point.x < m_left_pane_w )
	{
		...
		for( int i=0; i<m_Doc->m_fp_num_layers; i++ )
		{
			...
			if( r.PtInRect( point ) && i > LAY_BACKGND )
			{
				...
				m_Doc->m_fp_vis[i] = (int)!(m_Doc->m_fp_vis[i]);
	...
	if( pDC )
		ReleaseDC( pDC );
	Invalidate( FALSE );//OnLButtonDown
	CView::OnLButtonDown(nFlags, point);
}
...
void CFootprintView::HandleKeyPress(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	case CUR_FP_GROUP_SELECTED:
		...
		else if( fk == FK_FP_OP_MAKE_COPPER	)
		{
			...
			HighlightGroup();
			FootprintModified( TRUE );
			SetFKText( CUR_FP_GROUP_SELECTED );
		}
		...
		else if( fk == FK_FP_OP_MAKE_NOTES )
		{
			...
			for( int i=0; i<m_fp.m_outline_poly.GetSize(); i++ )
			{
				...
				int gl = Get_fp_Side( m_fp.m_outline_poly[m_sel_id.i].GetLayer() );
				if( gl == 0 )
					MoveToLayer(LAY_FP_NOTES_T);
				else if( gl == 1 )
					MoveToLayer(LAY_FP_NOTES_B);
			}
			HighlightGroup();
			FootprintModified( TRUE );
			SetFKText( CUR_FP_GROUP_SELECTED );
	...
	case CUR_FP_POLY_CORNER_SELECTED:
		...
		else if( fk == FK_FP_ALIGN_X )
		{
			...
			{
				for( int icor=0; icor<m_fp.m_outline_poly[m_sel_id.i].GetNumCorners(); icor++ )
				{
					if( icor != m_sel_id.ii )
					{
						int gX = m_fp.m_outline_poly[m_sel_id.i].GetX( icor );
						m_fp.m_outline_poly[m_sel_id.i].SetX( icor, gX - x0 + ppx );
					}
				}
			}
		...
		else if( fk == FK_FP_ALIGN_Y )
		{
			...
			if( AfxMessageBox( ps, MB_YESNO ) == IDYES )
			{
				for( int icor=0; icor<m_fp.m_outline_poly[m_sel_id.i].GetNumCorners(); icor++ )
				{
					if( icor != m_sel_id.ii )
					{
						int gY = m_fp.m_outline_poly[m_sel_id.i].GetY( icor );
						m_fp.m_outline_poly[m_sel_id.i].SetY( icor, gY - y0 + ppy );
					}
				}
			}
	...
	case CUR_FP_CENTROID_SELECTED:
		...
		else if( fk == FK_ARROW )
		{
			...
			// moving 3D
			m_fp.Move3dObject(&m_fp.m_openscad_code, -m_dx, -m_dy, m_Doc->m_units);
			m_fp.Move3dObject(&m_fp.m_openscad_hole, -m_dx, -m_dy, m_Doc->m_units);
			//
			m_fp.m_centroid_x += m_dx;
			m_fp.m_centroid_y += m_dy;
			m_fp.Draw( m_dlist, m_Doc->m_smfontutil );
			FootprintModified( TRUE );
		}
		break;
...
{
	int old_key_option = m_fkey_option[2];
	...
	{
	case CUR_FP_NONE_SELECTED:
		m_fkey_option[0] = FK_FP_ADD_PCB_HOLE;
	...
	case CUR_FP_GROUP_SELECTED:
		...
		else if( old_key_option == FK_FP_OP_MAKE_SILK )
			m_fkey_option[2] = FK_FP_OP_MAKE_NOTES;
		else
			m_fkey_option[2] = FK_FP_OP_MAKE_COPPER;
		...
		m_fkey_option[7] = FK_FP_SET_WIDTH;
		break;
	case CUR_FP_PAD_SELECTED:
		ppx = prevx;
		ppy = prevy;
	...
	case CUR_FP_POLY_SIDE_SELECTED:
		...
		//	m_fp.m_outline_poly[m_sel_id.i].GetOpenscadData()->GetLength() )
		{
			...
			if( Get_fp_Side( m_fp.m_outline_poly[m_sel_id.i].GetLayer() ) != 2 )
			{
...
void CFootprintView::OnContextMenu(CWnd* pWnd, CPoint point )
{
	...
	case CUR_FP_POLY_SIDE_SELECTED:
		...
		ASSERT(pPopup != NULL);
		g_cl = m_fp.m_outline_poly[m_sel_id.i].GetClosed();
		if( g_cl == 0 )
			pPopup->EnableMenuItem( ID_FP_OPENSCAD, MF_GRAYED );
...
void CFootprintView::OnPolylineSetOrigin()
{
	PushUndo();
	RECT pr = m_fp.m_outline_poly[m_sel_id.i].GetBounds();
	CPoint P( (pr.left+pr.right)/2, (pr.top+pr.bottom)/2 );
	MoveOrigin( P.x, P.y );
	m_org_x -= P.x;
	m_org_y -= P.y;
	CRect screen_r;
...
void CFootprintView::OnPolylineOpenScad()
{
	CDlg_OpenScad dlg;
	CString * s = m_fp.m_outline_poly.GetAt( m_sel_id.i ).GetOpenscadData();
	BOOL isEMPTY = (s->Find("_extrude")==-1);
	CString gr="", gz="";
	BOOL bHull = m_fp.SyncOpenscadGlobalParameters( &gr, &gz );
	dlg.Initialize( s, &gr, &gz, bHull );
	int ok = dlg.DoModal();
	if( ok == IDOK )
	{
		if (abs(dlg.m_extrusion_val) < BY_ZERO && abs(dlg.m_extrusion_rot) < BY_ZERO)
		{
			// remove data
			CString emp = "";
			m_fp.m_outline_poly.GetAt(m_sel_id.i).SetOpenscadData(&emp);
		}
		m_fp.SyncOpenscadGlobalParameters( &dlg.s_global_rot, &dlg.s_global_z, &dlg.m_fill_space );
		CString sep = "|";
		m_fp.m_outline_poly.GetAt( m_sel_id.i ).SetOpenscadData( dlg.GenerateOpenscadData( &sep ) );
		if( isEMPTY )
			m_fp.m_outline_poly[m_sel_id.i].SetVisible(FALSE);
		FootprintModified( TRUE );
	...
	if( ret == IDOK )
	{
		...
		if( AfxMessageBox( ps, MB_YESNO ) == IDYES )
		{
			for( int icor=0; icor<m_fp.m_outline_poly[m_sel_id.i].GetNumCorners(); icor++ )
			{
				...
				{
					int gX = m_fp.m_outline_poly[m_sel_id.i].GetX( icor );
					m_fp.m_outline_poly[m_sel_id.i].SetX( icor, gX + dx );
					int gY = m_fp.m_outline_poly[m_sel_id.i].GetY( icor );
					m_fp.m_outline_poly[m_sel_id.i].SetY( icor, gY + dy );
				}
			}
		}
		//
...
void CFootprintView::OnPolylineFirstCorner()
{
	PushUndo();
	m_fp.m_outline_poly[m_sel_id.i].MakeFirst(m_sel_id.ii);
	m_fp.m_outline_poly[m_sel_id.i].Draw( m_dlist );
	CancelSelection();
	FootprintModified( TRUE );
	Invalidate( FALSE );
...
{
	AddPolyline();
	Invalidate( FALSE );//OnAddPolyline
}
void CFootprintView::AddPolyline( id * m_id )
{
	...
	if( m_polyline_width > 0 )
	{
		...
		if( m_id )
		{
			cl = m_fp.m_outline_poly.GetAt( m_id->i ).GetClosed();
			CString * OSC_data = m_fp.m_outline_poly.GetAt( m_id->i ).GetOpenscadData();
			dlg.Initialize( m_units, m_polyline_width, cl, 0, OSC_data->GetLength() );
		}
		else
	...
	if( ret == IDOK )
	{
		if( m_id )
		{
			...
			{
				m_fp.m_outline_poly[m_id->i].SetW(max( dlg.GetWidth(), NM_PER_MIL ));
				if(m_fp.m_outline_poly[m_id->i].GetLayer() != LAY_FP_PAD_THRU )
...
void CFootprintView::MoveOrigin( int x, int y )
{
	...
	m_fp.Move3dObject( &m_fp.m_openscad_code, x, y, m_Doc->m_units );
	m_fp.Move3dObject( &m_fp.m_openscad_hole, x, y, m_Doc->m_units );
	m_fp.Draw( m_dlist, m_Doc->m_smfontutil );
	FootprintModified( TRUE );
}
...
void CFootprintView::OnCentroidEdit()
{
	...
	if( ret == IDOK )
	{
		...
		if( m_fp.m_centroid_type == CENTROID_DEFAULT )
		{
			...
			// moving 3D
			m_fp.Move3dObject(&m_fp.m_openscad_code, -(c.x - m_fp.m_centroid_x), -(c.y - m_fp.m_centroid_y), m_Doc->m_units);
			m_fp.Move3dObject(&m_fp.m_openscad_hole, -(c.x - m_fp.m_centroid_x), -(c.y - m_fp.m_centroid_y), m_Doc->m_units);
			//
		...
		{
			// moving 3D
			m_fp.Move3dObject(&m_fp.m_openscad_code, -(dlg.m_x - m_fp.m_centroid_x), -(dlg.m_y - m_fp.m_centroid_y), m_Doc->m_units);
			m_fp.Move3dObject(&m_fp.m_openscad_hole, -(dlg.m_x - m_fp.m_centroid_x), -(dlg.m_y - m_fp.m_centroid_y), m_Doc->m_units);
			//
...
void CFootprintView::OnAdd3DHole()
{
	CDlgAddHole dlg;
	dlg.Initialize(&m_fp);
	dlg.DoModal();
...
void CFootprintView::OnAdd3DModule()
{
	CDlgAddModule dlg;
	dlg.Initialize(&m_fp);
	dlg.DoModal();
...
void CFootprintView::GenerateOpenscadFile()
{
	...
	if( rf == -1 )
		return;
	rf = path.GetLength() - rf - 1;
	CWnd * wnd = FindWindow( NULL, path.Right(rf) + " - OpenSCAD" );
	if( wnd == NULL )
		wnd = FindWindow( NULL, path.Right(rf) + "* - OpenSCAD" );
	if( wnd )
	{

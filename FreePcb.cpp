...
BEGIN_MESSAGE_MAP(CFreePcbApp, CWinApp)
	...
	ON_COMMAND(ID_HELP_GOTOFAQ, OnHelpFAQ)
	...
	ON_COMMAND_EX_RANGE(ID_RUN_INFO_BOX, ID_RUN_IMP_BMP, OnUtils)
END_MESSAGE_MAP()
...
void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if( !pDX->m_bSaveAndValidate )
	{
		...
		// VERSION (key)
#ifdef _DEBUG
		m_edit_build.SetWindowText("2.4 Debug: 20   e-mail: freepcb@niconson.com"/* // VERSION "$WCREV$ Debug: ($WCDATE$)" */ );
#else
		m_edit_build.SetWindowText( "2.4 Release: 20   e-mail: freepcb@niconson.com"/* "$WCREV$ Debug: ($WCDATE$)" */ );
#endif
	}
}
...
BOOL CFreePcbApp::OnUtils( UINT CMD )
{
	SHELLEXECUTEINFO ShExecInfo;
	CString fn = m_Doc->m_app_dir;
	switch(CMD)
	{
	case ID_RUN_INFO_BOX:
		fn += "\\FPC_EXE\\FreePCB_InfoBox\\ObjectManager.exe";
		break;
	case ID_RUN_LIB_MAN:
		fn += "\\FPC_EXE\\FreePCB_Libraries\\ABC_library.exe";
		break;
	case ID_RUN_INDUCTOR:
		fn += "\\FPC_EXE\\FreePCB_Inductor\\GenInductor.exe";
		break;
	case ID_RUN_RND_CORNERS:
		fn += "\\FPC_EXE\\FreePCB_RndRects\\Rounding_Rect.exe";
		break;
	case ID_RUN_REPLACE:
		fn += "\\FPC_EXE\\FreePCB_Replace\\Replace.exe";
		break;
	case ID_RUN_SHOW_NET:
		fn += "\\FPC_EXE\\FreePCB_Nets\\Nets.exe";
		break;
	case ID_RUN_INSERT_NETS:
		fn += "\\FPC_EXE\\FreePCB_InsertNets\\InsertNets.exe";
		break;
	case ID_RUN_INSERT_REPORT:
		fn += "\\FPC_EXE\\FreePCB_InsertReport\\InsertReport.exe";
		break;
	case ID_RUN_EXP_TANGO:
		fn += "\\FPC_EXE\\FreePCB_ExportPCAD\\FPC_2_PCB.exe";
		break;
	case ID_RUN_EXP_DXF:
		fn += "\\FPC_EXE\\FreePCB_ExportDXF\\ExportDXF.exe";
		break;
	case ID_RUN_IMP_TANGO:
		fn += "\\FPC_EXE\\FreePCB_ImportPCAD\\PCB_2_FPC.exe";
		break;
	case ID_RUN_IMP_DXF:
		fn += "\\FPC_EXE\\FreePCB_ImportDXF\\DXF_2_FPC_FreePCB.exe";
		break;
	case ID_RUN_IMP_BMP:
		fn += "\\FPC_EXE\\FreePCB_ImportBMP\\BMP_2_FPC.exe";
		break;
	}
	if( m_Doc->m_project_open )
	{		
		CString buf_str = "ObjMan \"" + m_Doc->m_pcb_full_path + "\"";
		int size = sizeof(buf_str)*(buf_str.GetLength()+1);
		HANDLE cmd = GlobalAlloc(GMEM_FIXED,size);
		memcpy( cmd, buf_str, size );
	...
	if( m_Doc->m_project_modified &&
		(CMD==ID_RUN_LIB_MAN||
		 CMD==ID_RUN_RND_CORNERS||
		 CMD==ID_RUN_REPLACE||
		 CMD==ID_RUN_SHOW_NET||
		 CMD==ID_RUN_INSERT_NETS||
		 CMD==ID_RUN_INSERT_REPORT||
		 CMD==ID_RUN_EXP_TANGO||
		 CMD==ID_RUN_EXP_DXF))
	{
		// WARNING

...
void CFreePcbDoc::Generate_GCODE(CStdioFile* f, float swell, int hatch, BOOL bHOLES, BOOL bLASERMODE)
{
	...
	if (bLASERMODE == 0)
	{
		...
		if (m_units == MIL)
			...
			f->WriteString("F300\n");
		f->WriteString(";----------------------------------------\n");
		f->WriteString("; Corner positioning\n");
		///
...
int CFreePcbDoc::RemovingZeroLengthOrColinearSides( CPolyLine * ap )
{
	int reDr = 0;
	...
	while( net ) 
	{
		if( net->nareas > 0 )
		{
			...
			for( int ia=0; ia<net->nareas; ia++ )
			{
				...
				Total_num++;
				int reDraw = RemovingZeroLengthOrColinearSides(ap);
				if (reDraw)
				{
	...
	if( ok )
	{
		...
		{ // write CUBE for projection of packages
			...
			if (ok)
			{
				...
				file.WriteString(str);
				str.Format("module Draw_%s_CUBE(side, frozen, space)\n{\n", moduleName);
				file.WriteString(str);
				file.WriteString("  if (is_undef(cube_scaleX) || is_undef(cube_scaleY) || is_undef(cube_scaleZ))\n");
				file.WriteString("  {\n");
				str.Format("    translate([max_height_%s/2, max_height_%s/2, 0])\n", moduleName, moduleName);
				file.WriteString(str);
				str.Format("    translate([frozen?0:originX_%s, frozen?0:originY_%s, side?max_height_%s/2-board_h:-max_height_%s/2])\n", moduleName, moduleName, moduleName, moduleName);
				file.WriteString(str);
				str.Format("    cube(max_height_%s, center=true); \n", moduleName);
				file.WriteString(str);
				file.WriteString("  }\n");
				...
				str.Format("    translate([max_height_%s/2, max_height_%s/2, 0])\n", moduleName, moduleName);
				file.WriteString(str);
				str.Format("    translate([frozen?0:originX_%s, frozen?0:originY_%s, side?(max_height_%s/2*cube_scaleZ)-board_h:(-max_height_%s/2*cube_scaleZ) ])\n", moduleName, moduleName, moduleName, moduleName);
				file.WriteString(str);
				file.WriteString("    scale([cube_scaleX, cube_scaleY, cube_scaleZ])\n");
				str.Format("    cube(max_height_%s, center=true); \n", moduleName);
				file.WriteString(str);
				...
				file.WriteString(str);
				file.WriteString("      translate([0, 0, push_direction * spaceAxisForCube])\n");
				str.Format("      Draw_%s_CUBE(side, frozen, 0);\n", moduleName);
				file.WriteString(str);
				file.WriteString("    }\n");
				file.WriteString("  }\n");
				file.WriteString("}\n");
			...
			if( ok )
			{
				...
				for(int i=0; i<foots.GetSize(); i++)
				{
					...
					for( int isp=foots.GetAt(i).GetLength(); isp<maxlen; isp++ )
						spc += " ";
					str.Format( "drw_%s%s = true; // controls Draw_%s();\n", foots.GetAt(i), spc, foots.GetAt(i));
					Scadfile.WriteString( str );
				}
				//
				...
				Scadfile.WriteString(str);
				Scadfile.WriteString("    rotate([d?90:-90, 0, 0])\n");
				str.Format("    Draw_%s_CUBE(0, frozen, sector);\n", moduleName);
				Scadfile.WriteString(str);
				Scadfile.WriteString("}\n");
				...
				Scadfile.WriteString(str);
				Scadfile.WriteString("    rotate([0, d?90:-90, 0])\n");
				str.Format("    Draw_%s_CUBE(0, frozen, sector);\n", moduleName);
				Scadfile.WriteString(str);
				Scadfile.WriteString("}\n");
				...
				Scadfile.WriteString(str);
				str.Format("    Draw_%s_CUBE(d?1:0, frozen, sector);\n", moduleName);
				Scadfile.WriteString(str);
				Scadfile.WriteString("}\n");
				//
				str.Format(	"\n\n\n//// Drawing\n");
		...
		else
		{
			...
			if( ok )
			{
				...
				for(int i=0; i<foots.GetSize(); i++)
				{
					...
					if( edata.Find( str ) < 0 )
					{
						str.Format( "\ndrw_%s%s = true; // was added to the PCB", foots.GetAt(i), spc );
						Scadfile.WriteString( str );
					}
				}
		...
		{
			CStdioFile Scadfile;
			ok = Scadfile.Open(jsonpath, CFile::modeCreate | CFile::modeWrite);
			if (ok)
			{
				Scadfile.WriteString("{\n");
				Scadfile.WriteString("    \"fileFormatVersion\": \"1\", \n");
				...
				for (int iset = 0; iset < 14; iset++)
				{
					if(iset == 0)
						...
						Scadfile.WriteString("        \"2: projection of top copper\": {\n");
					else if (iset == 2)
						Scadfile.WriteString("        \"3: projection of bottom copper\": {\n");
					else if (iset == 3)
						Scadfile.WriteString("        \"4: projection of top packages\": {\n");
					else if (iset == 4)
						Scadfile.WriteString("        \"5: projection of bottom packages\": {\n");
					else if (iset == 5)
						Scadfile.WriteString("        \"6: lateral pcb projection\": {\n");
					else if (iset == 6)
						Scadfile.WriteString("        \"7: frontal pcb projection\": {\n");
					else if (iset == 7)
						Scadfile.WriteString("        \"8: custom lateral projection\": {\n");
					else if (iset == 8)
						Scadfile.WriteString("        \"9: custom frontal projection\": {\n");
					else if (iset == 9)
						Scadfile.WriteString("        \"10: custom combo projection\": {\n");
					else if (iset == 10)
						Scadfile.WriteString("        \"11: frontal 3D section\": {\n");
					else if (iset == 11)
						Scadfile.WriteString("        \"12: lateral 3D section\": {\n");
					else if (iset == 12)

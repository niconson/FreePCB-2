...
// Attributes
public:
	enum ProtectionConst {
		PROTECT_ON_GERBER=0,
		PROTECT_ARCHIVED, // DO NOT CHANGE (USED IN PCB EDITOR) !
		PROTECT,
		NO_PROTECT,
		AUTOPROTECT_NETLIST
	};
...
public:
	...
	int ReadOptions( CStdioFile * pcb_file, BOOL rColors, BOOL rCropData=0 );
	int ReadMessageFromSchematic( CString * file, BOOL SHOW_MESSAGE=TRUE );
	void WriteOptions( CStdioFile * pcb_file, BOOL bConfig );
...
public:
	...
	int m_pdf_rgb[MAX_LAYERS][3];	// array of RGB values for each drawing layer
...
public:
	...
	afx_msg void OnFileSaveVersion();
	void OnFileSaveBeforeImport();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
public:
	...
	afx_msg void FileImportNetlist( CString * filename );
	afx_msg void OnFileReloadNetlist();
	...
	void FilePrint( int m_height1, int m_height2, int m_height3, int m_no_hatch, int m_dsp_clrs, RECT * region = NULL );
	//
	...
	afx_msg void OnFileGenerate3DFile();
	afx_msg void OnProjectCombineNets();
	BOOL ProjectCombineNets( CNetList * nl ); 
	...
	void OnProtection();
	RECT GetBoardRect( int* Width = NULL, BOOL bSelLegalBoards = FALSE );
	RECT AddBoardHoles( BOOL bCANCEL=FALSE, POINT * MakePanel = NULL);
	void CancelBoardHoles( BOOL bUNDO=TRUE );
	void ClipClear();
	void OnAddTopImage();
	void OnAddBottomImage();
	void OnDeleteTopImage();
	void OnDeleteBottomImage();
	void OnInvertColors();
	void OnSwapImages();
};

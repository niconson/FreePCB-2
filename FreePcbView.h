...
public:
	...
	enum{C_JUMP=0, C_PARTS, C_PINS, C_SUFFIX, C_MOVE, C_PASTE, C_ENQUARY, C_INVALIDATE, C_UNSEL, C_SHOW, C_RELOAD};
...
public:
	...
	// if board is full hatch pattern
	int m_board_back;
...
public:
	...
	int SelectPart( cpart * part, int bPins=1 );
	...
	void SelectSimilarParts( BOOL byValues );
	void SelectMergeSegments( cpart * mP );
	void VertexMoved();
	void ForceVia();
	void ShowPinState(cpart * p, int pin);
	void JumpToPin( cpart * sel_p, id sid );

	int MoveAreaToLayer( int NewLayer=0 ) ;
	BOOL ShowMerges( int iTRANSPARENT=0 );
	BOOL DetachRatLines();
	BOOL PartHasNoInterPinTraces( cpart * p );
	BOOL SetDrawLayer( int L );
	...
	void ExplodeGroup();	
	int  NewSelect( void * ptr, id * sid, BOOL bSet_cursor_mode, BOOL bInvert, BOOL FindArc=TRUE );// Set sel element 
	...
	void HighlightGroup(int bPins=1);
	...
	void SelectBetween();
	void SelectContour( BOOL Unselect=0 );
...
public:
	...
	RECT ViewAllElements( BOOL bHilited=0 );
	afx_msg void OnRouteComplete();
	...
	afx_msg void OnCopyCutout();
	// alignments
	afx_msg void OnAlignSides();
	afx_msg void OnAlignX();
	afx_msg void OnAlignY();
	afx_msg void OnAlignXY();
	afx_msg void OnAlignBetweenX();
	afx_msg void OnAlignBetweenY();
	afx_msg void OnAlignBetweenXY();
	afx_msg void OnAlignMiddleX();
	afx_msg void OnAlignMiddleY();

...
{
	_int64 x_10 = (_int64)1000*(_int64)x/(_int64)NM_PER_MIL;	// 2.6
	_int64 y_10 = (_int64)1000*(_int64)y/(_int64)NM_PER_MIL;	// 2.6
	CString str;
	if( light_state == LIGHT_NONE )
		ASSERT(0);
	else
		str.Format( "X%I64dY%I64dD0%d*\n", x_10, y_10, light_state );
	f->WriteString( str );
}
	...
	else
	{
		...
		if( npo )
		{
				...
				t == CAperture::AP_OCTAGON )
			{
				...
				// 
				WriteMoveTo( f, rnd[0].x, rnd[0].y, LIGHT_ON );
				f->WriteString( "G37*\n" );
			}
			else if( t == CAperture::AP_RECT_THERMAL )
			{
...
#define PASS1 (ipass==1)
	...
	for( int ipass=0; ipass<2; ipass++ )
	{
		...
		{	//=============================================================================
			...
			f->WriteString( "%MOIN*%\n" );
			f->WriteString( "G04 " + layer_name_str + " *\n" );
			// macros
		...
		if (n_x > 1 || n_y > 1)
		{
				...
				((layer == LAY_PASTE_TOP || layer == LAY_PASTE_BOTTOM) && panel.m_reference.Right(1) == "2"))
			{
				...
				if (PASS1)
				{
					CPoint * rp = GetRepperPoint(0, &op_rect);
					::WriteMoveTo(f, rp->x, rp->y, LIGHT_FLASH);
					rp = GetRepperPoint(1, &op_rect);
					if (panel.m_ref_count == 0 || panel.m_ref_count == 2)
						::WriteMoveTo(f, rp->x, rp->y, LIGHT_FLASH);
					if (panel.m_ref_count == 1 || panel.m_ref_count == 2)
					{
						rp = GetRepperPoint(2, &op_rect);
						::WriteMoveTo(f, rp->x, rp->y, LIGHT_FLASH);
						rp = GetRepperPoint(3, &op_rect);
						::WriteMoveTo(f, rp->x, rp->y, LIGHT_FLASH);
					}
				}
		...
		if  ( layer == LAY_HILITE )
		{
			...
			while( el->next->next )
			{
				...
				else if( el->gtype == DL_RECT && el->transparent == 0 )
				{
				...
				else if(( el->gtype == DL_HOLLOW_RECT && el->transparent == 0 ) || ( el->gtype == DL_RECT && el->transparent ))
				{
					CAperture ap( CAperture::AP_CIRCLE, w, 0 );
					ChangeAperture( &ap, &current_ap, &ap_array, PASS0, f );
					if PASS1
			...
			if ( bYES )
			{
				...
				{	//=============================================================================
					...
					if ( PASS1 )
					{
						...
						for (int h=0; h<nh; h++)
						{
							dl_element * dlh = a->poly->GetHatchLoc(h);
							CArray<CPoint> * gP = dl->Get_Points( dlh, NULL, 0 );
							int npt = gP->GetSize();
							CPoint * P = new CPoint[npt];
		...
		if( (flags & GERBER_PILOT_HOLES) && pilot_diameter )
		{
			if( PASS1 )
			{			
				f->WriteString( "\nG04 ----------------------- Draw Pilot Holes (scratch)*\n" );
				if( layer == LAY_TOP_COPPER || layer == LAY_BOTTOM_COPPER || layer == LAY_SM_TOP || layer == LAY_SM_BOTTOM )
					SET_LPC
				else
					SET_LPD
				current_ap.m_type = CAperture::AP_NONE;	// force selection of aperture
				   ...
				   int n_x, int n_y, int space_x, int space_y )
{
		...
		while( part->next != 0 )
		{
			...
			if( s )
			{
				...
				for( int ip=0; ip<s->GetNumPins(); ip++ )
				{
					...
					{
						::AddToArray( abs(ps->hole_size)/NM_PER_MIL, &diameter );
						RET.x = ps->x_rel;
						RET.y = ps->y_rel;
					}
				}
	...
	for( int iid=0; iid<diameter.GetSize(); iid++ )
	{
		// write hole sizes
		int d = diameter[iid];
		str.Format( "T%02dC%5.3f\n", iid+1, (double)diameter[iid]/1000.0 ); 
		file->WriteString( str );
	}
	file->WriteString( "%\n" );		// start data
	...
	for( int id=0; id<diameter.GetSize(); id++ )
	{
		...
		if (n_x > 1 || n_y > 1)
		{
			if (d == panel.m_holes[0] / NM_PER_MIL)
			{
				...
				{
					y1 = (int)(panel.m_pcb_rect.bottom + (panel.m_fields[0] - panel.m_fields[1]*2) / 4) / (NM_PER_MIL / 10);
					y2 = (int)(panel.m_pcb_rect.bottom - (panel.m_fields[0] - panel.m_fields[1]*2) / 4 + range_y) / (NM_PER_MIL / 10);
				}
				str.Format("X%.6dY%.6d\n", x1, y1);
				file->WriteString(str);
				str.Format("X%.6dY%.6d\n", x1, y2);
						...
						if( s )
						{
							...
							for( int ip=0; ip<s->GetNumPins(); ip++ )
							{
								...
								if( ps->hole_size )
								{
									...
									if( d == abs(ps->hole_size)/NM_PER_MIL )
									{
										str.Format( "X%.6dY%.6d\n", 

...
void CMainFrame::DisableMenuItems( CMenu* pMenu )
{
	...
	submenu->EnableMenuItem( ID_FILE_SAVE_VERSION, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );	
	...
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE1, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE2, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE3, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE4, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE5, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE6, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	...
	submenu->EnableMenuItem( ID_FILE_GENERATE_3D, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	...
	submenu = pMenu->GetSubMenu(8);	// "Utils" submenu
	submenu->EnableMenuItem( ID_RUN_INFO_BOX, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );	
	submenu->EnableMenuItem( ID_RUN_RND_CORNERS, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_RUN_REPLACE, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_RUN_SHOW_NET, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_RUN_INSERT_NETS, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_RUN_INSERT_REPORT, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_RUN_EXP_TANGO, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	submenu->EnableMenuItem( ID_RUN_EXP_DXF, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
}

void CMainFrame::EnableMenuItems( CMenu* pMenu )
{
	...
	pMenu->EnableMenuItem( 8, MF_BYPOSITION | MF_ENABLED );
	CMenu* submenu = pMenu->GetSubMenu(0);	// "File" submenu
	...
	submenu->EnableMenuItem( ID_FILE_SAVE_VERSION, MF_BYCOMMAND | MF_ENABLED );	
	...
	if( theApp.m_Doc )
		if( theApp.m_Doc->m_netlist_full_path.GetLength() )
	...
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE1, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE2, MF_BYCOMMAND | MF_ENABLED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE3, MF_BYCOMMAND | MF_ENABLED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE4, MF_BYCOMMAND | MF_ENABLED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE5, MF_BYCOMMAND | MF_ENABLED);
	submenu->EnableMenuItem( ID_FILE_GENERATEDXFFILE6, MF_BYCOMMAND | MF_ENABLED);
	for (int im = 0; im < 14; im++)
	...
	submenu->EnableMenuItem( ID_FILE_GENERATE_3D, MF_BYCOMMAND | MF_ENABLED );	
	submenu->EnableMenuItem( ID_DSN_FILE_EXPORT, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );	
	submenu->EnableMenuItem( ID_SES_FILE_IMPORT, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED );
	...
	submenu = pMenu->GetSubMenu(8);	// "Utils" submenu
	submenu->EnableMenuItem( ID_RUN_INFO_BOX, MF_BYCOMMAND | MF_ENABLED );	
	submenu->EnableMenuItem( ID_RUN_RND_CORNERS, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_RUN_REPLACE, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_RUN_SHOW_NET, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_RUN_INSERT_NETS, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_RUN_INSERT_REPORT, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_RUN_EXP_TANGO, MF_BYCOMMAND | MF_ENABLED );
	submenu->EnableMenuItem( ID_RUN_EXP_DXF, MF_BYCOMMAND | MF_ENABLED );
}
...
int CMainFrame::OnCopyData(CWnd* pWnd, COPYDATASTRUCT* Msg) 
{
	...
	{
		key_str = "jump";
		mode = CFreePcbView::C_JUMP;
	}
	else if(Mess.Left(10) == "cancel_sel")
	{
		key_str = "cancel_sel";
		mode = CFreePcbView::C_UNSEL;
	}
	else if(Mess.Left(8) == "show_sel")
	{
		key_str = "show_sel";
		mode = CFreePcbView::C_SHOW;
	}
	else if (Mess.Left(4) == "pins")
	{
		key_str = "pins";
		mode = CFreePcbView::C_PINS;
	}
	else if (Mess.Left(5) == "parts")
	{
		key_str = "parts";
		mode = CFreePcbView::C_PARTS;
	}
	else if (Mess.Left(6) == "suffix")
	{
		key_str = "suffix";
		mode = CFreePcbView::C_SUFFIX;
	}
	else if (Mess.Left(4) == "move")
	{
		key_str = "move";
		mode = CFreePcbView::C_MOVE;
	}
	else if (Mess.Left(5) == "paste")
	{
		key_str = "paste";
		mode = CFreePcbView::C_PASTE;
	}
	else if (Mess.Left(7) == "enquary")
	{
		key_str = "enquary";
		mode = CFreePcbView::C_ENQUARY;
	}
	else if(Mess.Left(10) == "invalidate")
	{
		key_str = "invalidate";
		mode = CFreePcbView::C_INVALIDATE;
	}
	else if(Mess.Left(13) == "check_netlist")
	{
		key_str = "check_netlist";
		mode = CFreePcbView::C_RELOAD;
	}
	if (mode >= 0)
	{
		...
		m_view = (CView*)GetActiveView();
		if( (CFreePcbView*)m_view )
			((CFreePcbView*)m_view)->OnInfoBoxMess(mode,np,&p,IronScale);
	}
	return 0;
}
...
void CMainFrame::OnTimer(UINT nIDEvent)
{
	...
	if( m_view->IsKindOf( RUNTIME_CLASS(CFreePcbView) ) )
	{
		...
		{
			if( nIDEvent == DRAWING_TIMER )
			{
				SetTimer( DRAWING_TIMER, INT_MAX, 0 );
				doc->m_view->SetDrawLayer( ENABLE_CHANGE_DRAW_LAYER );
				m_view->Invalidate(FALSE);
			}
			else if( nIDEvent == JUMPING_TIMER )

...
//
void CNetList::CancelNextNet( BOOL bRST )
{
	if( bRST )
		m_pos_i = 0;
	else
...
int CNetList::GenPolygonConnect( cnet * net, int icon, int layer, int clrnc, CArray<CPoint> * A )
{
	A->SetSize(0);
	CPoint p, pp;
	int Step = 0;
	int w = net->connect[icon].seg[0].width;
	int l = net->connect[icon].seg[0].layer;
	if( l != layer )
		return 0;
	int nS = net->connect[icon].nsegs;
	for( int ii=0; ii>=0; Step<=nS?ii++:ii-- )
	{
		if( net->connect[icon].vtx[ii].via_hole_w )
			return 0;
		int x = net->connect[icon].vtx[ii].x;
		int y = net->connect[icon].vtx[ii].y;
		if( Step < nS )
			if( w != net->connect[icon].seg[ii].width || l != net->connect[icon].seg[ii].layer )
				return 0;
		if( (ii > 1 && Step <= nS) || (ii < (nS-1) && Step > nS ) )
		{
			double ang1 = Angle(p.x,p.y,pp.x,pp.y);
			double ang2 = Angle(x,y,p.x,p.y);
			CPoint np(x,y);
			double dang = Angle(pp,p,np);
			if( dang > 91.0 || dang < -91.0 )
				return 0;
			//
			//
			double newL = (clrnc+w/2)/cos(dang*M_PI/360.0);
			if( newL < 0 )
				newL = -newL;
			CPoint out;
			double tar_ang = (ang1+90.0+dang/2.0);
			out.x = (double)p.x + newL*cos((tar_ang)*M_PI/180.0);
			out.y = (double)p.y + newL*sin((tar_ang)*M_PI/180.0);
			A->Add(out);
			//
		}
		if( ii == nS || ( Step && ii == 0 ))
		{
			...
			double newL = (clrnc+w/2);
			CPoint out;
			out.x = (double)x + newL*cos((ang+90.0)*M_PI/180.0);
			out.y = (double)y + newL*sin((ang+90.0)*M_PI/180.0);
			A->Add(out);
			out.x = (double)x + newL*cos((ang-90.0)*M_PI/180.0);
			out.y = (double)y + newL*sin((ang-90.0)*M_PI/180.0);
			A->Add(out);
...
void CNetList::AddCutout(	CArray<gpc_polygon*> * Cutouts, 
							CArray<RECT> * Rects, 
							gpc_polygon ** gpc_union,
							gpc_polygon * gpc_tmp, 
							gpc_polygon * gpc_result,
							RECT * currR )
{
	static CRect staticR = rect( CPoint(INT_MAX,INT_MAX), CPoint(INT_MIN,INT_MIN) );
	if( currR == NULL )
	{
		if( gpc_union[0]->num_contours )
		{
			Cutouts->Add( gpc_union[0] );
			Rects->Add( staticR );
			gpc_union[0] = new gpc_polygon;
			gpc_union[0]->contour = NULL;
			gpc_union[0]->hole = NULL;
			gpc_union[0]->num_contours = 0;
		}
		if( gpc_tmp->num_contours )
			gpc_free_polygon( gpc_tmp );
		if( gpc_result->num_contours )
			gpc_free_polygon( gpc_result );
		// reset rect
		staticR = rect( CPoint(INT_MAX,INT_MAX), CPoint(INT_MIN,INT_MIN) );
		return;
	}
	if( gpc_result->num_contours == 1 )
	{
		SwellRect( &staticR, *currR );
		if( gpc_union[0]->num_contours )
			gpc_free_polygon( gpc_union[0] );
		//
		gpc_union[0]->contour = gpc_result->contour;
		gpc_union[0]->hole = gpc_result->hole;
		gpc_union[0]->num_contours = gpc_result->num_contours;
		//
	...
	else 
	{
		...
		staticR = *currR;
		//
		gpc_union[0]->contour = gpc_tmp->contour;
		gpc_union[0]->hole = gpc_tmp->hole;
		gpc_union[0]->num_contours = gpc_tmp->num_contours;
		//
...
//AddCutoutsForArea
int CNetList::AddCutoutsForArea(	cnet * area_net, 
									int ia, 
									cnet * crop_net, 
									int pad_cl, 
									int seg_cl,
...
{	
	int PERCENT = 50;
	//
	CPolyLine * thisPoly = area_net->area[ia].poly;
	//
	RECT aR =	thisPoly->GetBounds();
	int aL =	thisPoly->GetLayer();
	int gnc =	thisPoly->GetNumCorners();
	int aW =	thisPoly->GetW();
	int aM =	thisPoly->GetMerge();
	int aM_cl = m_ml->GetClearance(aM);
	if( getbit( selection_flags, MC_VALIDATE ) )
	{
		aW = 0;
	}
	if( thrml_w <= aW )
	...
	gpc_polygon * gpc_union = new gpc_polygon;
	gpc_union->num_contours = 0;
	gpc_union->hole = NULL;
	gpc_union->contour = NULL;
	//
	gpc_polygon gpc_tmp;
	gpc_tmp.num_contours = 0;
	gpc_tmp.hole = NULL;
	...
	//
	BOOL OCTAGON = getbit( selection_flags, MC_OCTAGON );
	for( cnet * n = GetFirstNet(); n; n = GetNextNet(/*LABEL*/) )
	{
		if( getbit( selection_flags, MC_AREAS ) ) 
		{
			if( n == crop_net || getbit( selection_flags, MC_ALL_NETS ) )
			{
				for( int i=0; i<n->nareas; i++ )
				{
					if( i == ia && n == area_net )
						continue;
					CPolyLine * crop_p = n->area[i].poly;
					int cropW = crop_p->GetW();
					if( crop_p->GetHatch() == CPolyLine::NO_HATCH && abs(cropW) < _2540 )
						continue;
					int L = crop_p->GetLayer();
					if( aL == L || getbit( selection_flags, MC_ALL_LAYERS ) )
					{
						RECT gR = crop_p->GetBounds();
						int Clearance = area_cl*2.0 + aW + cropW;
						SwellRect( &gR, Clearance );
						if( RectsIntersection( aR, gR ) != -1 )
						{
							if( crop_p->GetMerge() >= 0 )
							{
								int mer_cl = m_ml->GetClearance(crop_p->GetMerge());
								if( mer_cl > 0 )
									Clearance = mer_cl*2.0 + aW + cropW;
							}
							if( aM_cl > 0 )
								Clearance = aM_cl*2.0 + aW + cropW;
							Clearance += (Clearance/PERCENT);
							for( int icr=0; icr<crop_p->GetNumCorners(); icr++ )
							{
								int inext = crop_p->GetIndexCornerNext(icr);
								int x1 = crop_p->GetX(icr);
								int y1 = crop_p->GetY(icr);
								int x2 = crop_p->GetX(inext);
								int y2 = crop_p->GetY(inext);
								RECT segR = rect( x1,y1,x2,y2 );
								SwellRect( &segR, Clearance/2 );
								if( RectsIntersection( aR, segR ) != -1 )
								{
									int s_style = crop_p->GetSideStyle( icr );
									CPoint PO[N_SIDES_APPROX_ARC+1];
									int n_sides = Generate_Arc( x1, y1, x2, y2,
																s_style, 
																PO, 
																N_SIDES_APPROX_ARC );
									//
									for( int iel=0; iel<n_sides-1; iel++ )
									{	
										CPoint CONT[20];
										int npo = Gen_HollowLinePoly( PO[iel].x, PO[iel].y, PO[iel+1].x, PO[iel+1].y, Clearance, CONT, 20 );
										//
										g_v_list->num_vertices = npo;
										g_v_list->vertex = (gpc_vertex*)calloc( npo, 2*sizeof(double) );	
										for( int ico=0; ico<npo; ico++ )
										{
											g_v_list->vertex[ico].x = CONT[ico].x;
											g_v_list->vertex[ico].y = CONT[ico].y;	
										}
										gpc_add_contour( &gpc_tmp, g_v_list, 0 );
										gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
										AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &segR );
		...
		if( getbit( selection_flags, MC_SEGS ) || getbit( selection_flags, MC_HOLES ) )
		{
			if( n == crop_net || ( n != area_net && getbit( selection_flags, MC_ALL_NETS ) ) )
			{
				for( int i=0; i<n->nconnects; i++ )
				{
					...
					if( aM_cl > 0 )
						mer_cl = aM_cl;
					//CArray<CPoint> polyconn;
					//if( GenPolygonConnect( n, i, aL, mer_cl?mer_cl:seg_cl, &polyconn ) > 2 )
					//{
					//	int npo = polyconn.GetSize();
					//	g_v_list->num_vertices = npo;
					//	g_v_list->vertex = (gpc_vertex*)calloc( npo, 2*sizeof(double) );	
					//	for( int ico=0; ico<npo; ico++ )
					//	{
					//		g_v_list->vertex[ico].x = polyconn[ico].x;
					//		g_v_list->vertex[ico].y = polyconn[ico].y;	
					//	}
					//	gpc_add_contour( &gpc_tmp, g_v_list, 0 );
					//	gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
					//	AddCutout( &Cutouts, &gpc_union, &gpc_tmp, &gpc_result );
					//	delete g_v_list->vertex;
					//}
					//else 
					{		
						for( int ii=0; ii<n->connect[i].nsegs; ii++ )
						{
							...
							int wSeg = n->connect[i].seg[ii].width;
							SwellRect( &sR, wSeg/2 );
							int best_cl = seg_cl;
							if( mer_cl > 0 )
								best_cl = mer_cl;
							SwellRect( &sR, best_cl+aW/2.0 );
							if( (vW-hW)/2.0 < hole_cl-best_cl && getbit( selection_flags, MC_HOLES ) )
								SwellRect( &vR, hole_cl+aW/2.0 );
							else
								SwellRect( &vR, best_cl+aW/2.0 );
							if( RectsIntersection( aR, vR ) != -1 && hW && getbit( selection_flags, MC_HOLES ) )
							{
								int CS=0;
								GetViaPadInfo( n, i, ii+1, aL, &vW, &hW, &CS );
								int Clearance1 = seg_cl*2.0 + vW + aW;
								int Clearance2 = hole_cl*2.0 + hW + aW;
								if( mer_cl > 0 )
									Clearance1 = mer_cl*2.0 + vW + aW;
								if( getbit( selection_flags, MC_SEGS ) == 0 )
									Clearance1 = 0;
								int Clearance = max(Clearance1,Clearance2);
								Clearance += (Clearance/PERCENT);
								CPoint CONT[20];
								int npo = Gen_RndRectPoly( n->connect[i].vtx[ii+1].x, n->connect[i].vtx[ii+1].y, 
												 Clearance, Clearance, OCTAGON?Clearance/3.414:Clearance/2.0, 0, CONT, OCTAGON?8:20 );
								//
								g_v_list->num_vertices = npo;
								g_v_list->vertex = (gpc_vertex*)calloc( npo, 2*sizeof(double) );	
								for( int ico=0; ico<npo; ico++ )
								...
								gpc_add_contour( &gpc_tmp, g_v_list, 0 );
								gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
								AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &vR );
								delete g_v_list->vertex;
							}
							if( RectsIntersection( aR, sR ) != -1 && getbit( selection_flags, MC_SEGS ) )
							{
								if( n->connect[i].seg[ii].layer == aL || getbit( selection_flags, MC_ALL_LAYERS ) ) 
								{
									int CS=0;
									GetViaPadInfo( n, i, ii+1, aL, &vW, &hW, &CS );
									int Clearance;
									if( mer_cl > 0 )
										Clearance = mer_cl*2.0 + n->connect[i].seg[ii].width + aW;
									else
										Clearance = seg_cl*2.0 + n->connect[i].seg[ii].width + aW;
									Clearance += (Clearance/PERCENT);
									CPoint CONT[20];
									int npo = Gen_HollowLinePoly( n->connect[i].vtx[ii].x, n->connect[i].vtx[ii].y, 
									...
									gpc_add_contour( &gpc_tmp, g_v_list, 0 );
									gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
									AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &sR );
									delete g_v_list->vertex;
								}
							}
						}
			...
			else if( n == area_net && ( cam_flags & GERBER_NO_VIA_THERMALS ) == 0 )
			{
				for( int i=0; i<n->nconnects; i++ )
				{
					for( int seg=1; seg<=n->connect[i].nsegs; seg++ )
					{
						cconnect * con = &n->connect[i];
						int lbit = con->vtx[seg].layer_bit;
						///if( con->vtx[seg].via_w )
						if( getbit( lbit, aL ) )
						{
							int px = con->vtx[seg-1].x;
							int py = con->vtx[seg-1].y;
							int vx = con->vtx[seg].x;
							int vy = con->vtx[seg].y;
							int via_angle = ((abs(px-vx)<NM_PER_MIL||abs(py-vy)<NM_PER_MIL)?0:45);
							CPoint CONT[20];
							int Clearance = thrml_cl + aW/2.0; 
							int npo = Gen_RndRectPoly(	vx, vy, con->vtx[seg].via_w+Clearance*2, con->vtx[seg].via_w+Clearance*2, 
														(con->vtx[seg].via_w/2.0+Clearance)/2.0, via_angle, CONT, 20, thrml_w-aW );
							RECT vr = rect(vx,vy,vx,vy);
							SwellRect( &vr, con->vtx[seg].via_w-2 );
							int tC = npo/4;
							for( int step=0; step<4; step++ )
							{
								g_v_list->num_vertices = tC;
								g_v_list->vertex = (gpc_vertex*)calloc( tC, 2*sizeof(double) );	
								for( int ico=step*tC; ico<(step+1)*tC; ico++ )
								{
									g_v_list->vertex[ico%tC].x = CONT[ico].x;
									g_v_list->vertex[ico%tC].y = CONT[ico].y;	
								}
								gpc_add_contour( &gpc_tmp, g_v_list, 0 );
								gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
								AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &vr );
	...
	for( cpart * p=m_plist->GetFirstPart(); p; p=m_plist->GetNextPart(p) )
	{
		if( p->shape )
		{
			...
			if( aM_cl > 0 )
				mer_cl = aM_cl;
			if( getbit( selection_flags, MC_PADS ) || getbit( selection_flags, MC_HOLES ) )
			{
				for( int ip=0; ip<p->shape->GetNumPins(); ip++ )
				{
					...
					if( mer_cl > 0 )
						SwellRect( &pb, mer_cl+aW/2.0 );
					else
						SwellRect( &pb, pad_cl+aW/2.0 );
					if(RectsIntersection( aR, pb ) != -1 )
					{
						for( int step_layer=(getbit( selection_flags, MC_ALL_LAYERS )?LAY_TOP_COPPER:aL); 
								 step_layer<(getbit( selection_flags, MC_ALL_LAYERS )?LAY_TOP_COPPER+3:aL+1); 
								 step_layer++ )
						{
							int Clearance = pad_cl + aW/2.0;
							int pad_type=0, pad_x=0, pad_y=0, pad_w=0, pad_l=0, pad_r=0, pad_hole=0, pad_angle=0, pad_connect=0, connect_f=0;
							cnet * pad_net=NULL;
							int check = m_plist->GetPadDrawInfo( p, ip, step_layer, 0, 0, 0, 0,
																&pad_type, &pad_x, &pad_y, &pad_w, &pad_l, &pad_r, 
																&pad_hole, &pad_angle, &pad_net, &pad_connect, &connect_f );
							if( check == 0 )
								continue;
							if( getbit( selection_flags, MC_PADS ) == 0 )
								pad_w = pad_l = 0;
							...
							if( pad_type == PAD_NONE || getbit( selection_flags, MC_PADS ) == 0 )
							{
								//pad_w = 0;
								//pad_l = 0;
								if( pad_hole == 0 )
								...
								pad_type == PAD_ROUND )
							{
								if( OCTAGON )
								{
									pad_r = min( pad_l, pad_w )/3.414;
									pad_type = PAD_OCTAGON;
								}
								else
									pad_r = min( pad_l, pad_w )/2.0;
							}
							if( OCTAGON && 
								(pad_type == PAD_RECT || pad_type == PAD_RRECT || pad_type == PAD_SQUARE) )
							{
								pad_r /= 2.0;
								pad_type = PAD_OCTAGON;
							}
							...
							if( pad_w == 0 && getbit( selection_flags, MC_HOLES ) == 0 )
								continue; // Upps
							if( pad_hole == 0 && getbit( selection_flags, MC_PADS ) == 0 )
								continue; // Upps
							//
							if( mer_cl > 0 )
								Clearance = mer_cl + aW/2.0;
							//
							if( pad_net == crop_net || ( pad_net != area_net && getbit( selection_flags, MC_ALL_NETS ) ) ||
							( getbit( selection_flags, MC_NO_THERMAL ) == 0 &&
							( (connect_f == ::PAD_CONNECT_DEFAULT && bSMT_copper_connect == 0) || connect_f == ::PAD_CONNECT_NEVER )))	
							{		
								{
									// when the size of the hole and the 
									// clearance of the hole in the 
									// total are larger than the size 
									// of the pad and the clearance, 
									// then we will set the maximum clearance
									//
									if( pad_w == 0 || pad_l == 0 )
									{
										pad_w = pad_hole;
									...
									if( pad_hole && getbit( selection_flags, MC_HOLES ) )
									{
										int hole_test = pad_hole + hole_cl*2.0 + aW;
										if( (Clearance*2.0+pad_l) < hole_test )
											Clearance = (hole_test - pad_l)/2.0;
										if( (Clearance*2.0+pad_w) < hole_test )
											Clearance = (hole_test - pad_w)/2.0;
									}
									Clearance += ((pad_r+Clearance)/PERCENT/2);
									if( pad_w )
									{
										if( pad_r )
											pad_r -= (pad_r/20);
								 		CPoint CONT[20];
										int npo = Gen_RndRectPoly(	pad_x, pad_y, pad_l+Clearance*2, pad_w+Clearance*2, 
																	(pad_r+Clearance<=NM_PER_MIL)?0:(pad_type==PAD_OCTAGON?pad_r+Clearance/2.0:pad_r+Clearance), 
																	-pad_angle, CONT, 
																	(pad_r+Clearance<=NM_PER_MIL)?4:(pad_type==PAD_OCTAGON?8:20) );
										//
										g_v_list->num_vertices = npo;
										g_v_list->vertex = (gpc_vertex*)calloc( npo, 2*sizeof(double) );	
										for( int ico=0; ico<npo; ico++ )
										...
										gpc_add_contour( &gpc_tmp, g_v_list, 0 );
										gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
										AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &pb );
										delete g_v_list->vertex;
									}
								}
							}
							else if( pad_net != crop_net && pad_net == area_net && getbit( selection_flags, MC_NO_THERMAL ) == 0 )
							{
								if( getbit( selection_flags, MC_THERMAL ) ||
								   (connect_f == ::PAD_CONNECT_DEFAULT && ( pad_hole&&!(cam_flags & GERBER_NO_PIN_THERMALS) ||
																		   !pad_hole&&!(cam_flags & GERBER_NO_SMT_THERMALS))) ||
									connect_f == ::PAD_CONNECT_THERMAL )
								{
									Clearance = thrml_cl + aW/2.0; 
									//if( cam_flags & GERBER_90_THERMALS )
									{
										if( pad_type != PAD_OCTAGON )
											pad_r = pad_r/2;
										if( pad_type == PAD_ROUND || pad_type == PAD_OVAL )
										{
											pad_r = min(pad_w,pad_l)/3.414;
											pad_type = PAD_OCTAGON;
										}
										if( pad_w && (pad_r+Clearance) > NM_PER_MIL && thrml_w > aW )
										{
									 		CPoint CONT[20];
											int npo = Gen_RndRectPoly(	pad_x, pad_y, pad_l+Clearance*2, pad_w+Clearance*2, 
																		pad_r+Clearance/2.0, -pad_angle, CONT, 20, thrml_w-aW );
											//
											int tC = npo/4;
											for( int step=0; step<4; step++ )
											{
			...
			if( getbit( selection_flags, MC_SEGS ) )
			{
				for( int io=0; io<p->m_outline_stroke.GetSize(); io++ )
				{
					RECT dl_r;
					m_dlist->Get_Rect( p->m_outline_stroke[io], &dl_r );
					if( mer_cl > 0 )
						SwellRect( &dl_r, mer_cl+aW/2.0 );
					else
						SwellRect( &dl_r, default_cl+aW/2.0 );
					if(RectsIntersection( aR, dl_r ) == -1 )
						continue;
					if( getbit( p->m_outline_stroke[io]->layers_bitmap, aL ) == 0 && getbit( selection_flags, MC_ALL_LAYERS ) == 0 )
						continue;
					...
					if( arr )
					{
						...
						m_dlist->Get_Points( p->m_outline_stroke[io], PTS, &npts );
						int w = p->m_outline_stroke[io]->el_w*m_pcbu_per_wu;
						int Clearance = default_cl*2 + aW + w;
						if( mer_cl > 0 )
							Clearance = mer_cl*2 + aW + w;
						//
						if( p->m_outline_stroke[io]->gtype == DL_POLYGON || p->m_outline_stroke[io]->gtype == DL_POLYLINE )
						{
							Clearance += (Clearance/PERCENT);
							//
							CPoint CONT[20];
							for( int iel=0; iel+1<npts; iel++ )
							{	
								int npo = Gen_HollowLinePoly( PTS[iel].x, PTS[iel].y, PTS[iel+1].x, PTS[iel+1].y, Clearance, CONT, 20 );
								//
								g_v_list->num_vertices = npo;
								g_v_list->vertex = (gpc_vertex*)calloc( npo, 2*sizeof(double) );	
								for( int ico=0; ico<npo; ico++ )
								...
								gpc_add_contour( &gpc_tmp, g_v_list, 0 );
								gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
								AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &dl_r );
								delete g_v_list->vertex;
							}
							if( p->m_outline_stroke[io]->gtype == DL_POLYGON )
							{
								//
								g_v_list->num_vertices = npts;
								g_v_list->vertex = (gpc_vertex*)calloc( npts, 2*sizeof(double) );	
								for( int ico=0; ico<npts; ico++ )
								{
									g_v_list->vertex[ico].x = PTS[ico].x;
									g_v_list->vertex[ico].y = PTS[ico].y;	
								}
								gpc_add_contour( &gpc_tmp, g_v_list, 0 );
								gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
								AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &dl_r );
	...
	if( getbit( selection_flags, MC_POLYLINES ) || getbit( selection_flags, MC_BOARD ) )
	{
		for( dl_element * dl=m_dlist->Get_Start()->next; dl; dl=dl->next )
		{
			if( dl == m_dlist->Get_End() )
				break;
			BOOL ok = 0;
			if( getbit( selection_flags, MC_POLYLINES ) && getbit( dl->layers_bitmap, LAY_SCRIBING ) )
				ok = 1;
			if( getbit( selection_flags, MC_BOARD ) && getbit( dl->layers_bitmap, LAY_BOARD_OUTLINE ) )
				ok = 1; 
			if( getbit( selection_flags, MC_BOARD ) && getbit( dl->layers_bitmap, LAY_PAD_THRU ) )
				ok = 1; 
			if( ok != 1 )
				continue;
			RECT dl_r;
			m_dlist->Get_Rect( dl, &dl_r );
			int cl = max( board_cl,default_cl );
			SwellRect( &dl_r, cl+aW/2.0 );
			if(RectsIntersection( aR, dl_r ) == -1 )
				continue;
			//
			CArray<CPoint> *arr = m_dlist->Get_Points( dl, NULL, 0 );
			if( arr )
			{
				int mer_cl = 0;
				cpart * dl_p = (cpart*)dl->ptr;
				if( dl_p )
					mer_cl = m_ml->GetClearance( dl_p->m_merge );
				if( aM_cl > 0 )
					mer_cl = aM_cl;
				int npts = arr->GetSize();
				CPoint * PTS = new CPoint[npts];//ok
				m_dlist->Get_Points( dl, PTS, &npts );
				...
				{
					Clearance += (Clearance/PERCENT);
					for( int ipts=0; ipts<(npts==2?1:npts); ipts++ )
					{
						CPoint PO[N_SIDES_APPROX_ARC+1];
						int n_sides = Generate_Arc( PTS[ipts].x, 
													PTS[ipts].y, 
													PTS[ipts<(npts-1)?(ipts+1):0].x, 
													PTS[ipts<(npts-1)?(ipts+1):0].y, 
													(dl->gtype == DL_ARC_CW)?(CPolyLine::ARC_CW):
													(dl->gtype == DL_ARC_CCW)?(CPolyLine::ARC_CCW):
													(CPolyLine::STRAIGHT), 
													PO, 
													N_SIDES_APPROX_ARC );
						//
	...
	if( getbit( selection_flags, MC_TEXTS ) )
	{
		for( dl_element * dl=m_dlist->Get_Start()->next; dl; dl=dl->next )
		{
			if( dl == m_dlist->Get_End() )
				break;
			if( getbit( dl->layers_bitmap, aL ) == 0 )
				continue;
			if( dl->id.st != ID_TXT )
				continue;
			RECT dl_r;
			...
			if( arr )
			{
				...
				m_dlist->Get_Points( dl, PTS, &npts );
				int w = dl->el_w*m_pcbu_per_wu;
				int Clearance = text_cl*2 + aW + w;
				if( mer_cl > 0 )
					Clearance = mer_cl*2 + aW + w;
				if( dl->gtype == DL_LINES_ARRAY )
				{
					Clearance += (Clearance/PERCENT);
					//
					for( int iel=0; iel<npts-1; iel+=2 )
					{	
						CPoint CONT[20];
						int npo = Gen_HollowLinePoly( PTS[iel].x, PTS[iel].y, PTS[iel+1].x, PTS[iel+1].y, Clearance, CONT, 20 );
						//
	...
	/*if( thisPoly->GetW() < 0 && getbit( selection_flags, MC_VALIDATE ) )
	{
		int gce = thisPoly->GetContourEnd(0);
		for( int i=gce; i>=0; i-- )
		{
			CPoint PO[N_SIDES_APPROX_ARC+1];
			int n_sides = Generate_Arc( thisPoly->GetX(i), 
										thisPoly->GetY(i), 
										thisPoly->GetX(thisPoly->GetIndexCornerNext(i)), 
										thisPoly->GetY(thisPoly->GetIndexCornerNext(i)), 
										thisPoly->GetSideStyle(i), 
										PO, 
										N_SIDES_APPROX_ARC );
			for(int ii=0; ii<n_sides-1; ii++ )
			{
				CPoint CONT[10];
				int npo = Gen_HollowLinePoly( PO[ii].x, PO[ii].y, PO[ii+1].x, PO[ii+1].y, -thisPoly->GetW(), CONT, 10 );
				//
				RECT r = rect( PO[ii].x, PO[ii].y, PO[ii+1].x, PO[ii+1].y );
				SwellRect( &r, -thisPoly->GetW()/2 );
				//
				g_v_list->num_vertices = npo;
				g_v_list->vertex = (gpc_vertex*)calloc( npo, 2*sizeof(double) );	
				for( int ico=0; ico<npo; ico++ )
				...
				gpc_add_contour( &gpc_tmp, g_v_list, 0 );
				gpc_polygon_clip( GPC_UNION, gpc_union, &gpc_tmp, &gpc_result );
				AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, &r );
				delete g_v_list->vertex;
			}
		}
		thisPoly->SetW(0);
	...
	AddCutout( &Cutouts, &Rects, &gpc_union, &gpc_tmp, &gpc_result, NULL );
	//
	//test
	if( Rects.GetSize() != Cutouts.GetSize() )
		ASSERT(0);
	// combining cutouts
	...
	for( int icut1=Cutouts.GetSize()-2; icut1>0; icut1-- )
	{
		...
		{
			if( RectsIntersection( Rects[icut1], Rects[icut2] ) >= 0 )
			{
				gpc_polygon_clip( GPC_UNION, Cutouts[icut1], Cutouts[icut2], clip );
				if( clip->num_contours == 1 )
				{
					gpc_free_polygon( Cutouts[icut1] );
					gpc_free_polygon( Cutouts[icut2] );
					delete Cutouts[icut1];
					Cutouts[icut1] = clip;
					delete Cutouts[icut2];
					Cutouts.RemoveAt(icut2);
					SwellRect( &Rects[icut1], Rects[icut2] );
					Rects.RemoveAt(icut2);
					clip = new gpc_polygon;
					clip->num_contours = 0;
					clip->contour = NULL;
					clip->hole = NULL;
				}
	...
	//
	//
	for( int icut=0; icut<Cutouts.GetSize(); icut++ )
	{
		for( int icor=0; icor<Cutouts.GetAt(icut)->contour[0].num_vertices; icor++ )
			AppendAreaCorner( area_net, ia, Cutouts.GetAt(icut)->contour[0].vertex[icor].x, Cutouts.GetAt(icut)->contour[0].vertex[icor].y, 0, 0 );
		thisPoly->Close( 0, 0 );
	}
	for( int icut=0; icut<Cutouts.GetSize(); icut++ )
	{
		gpc_free_polygon( Cutouts[icut] );
		delete Cutouts[icut];
	}
	//test
	if( Rects.GetSize() != Cutouts.GetSize() )
	...
	thisPoly->Draw();
	if( m_dis_hatch )
		area_net->area[ia].poly->SetHatchVisible(0); // 
	return 0;
}
...
//
void CNetList::RemoveNetPin( cnet * net, CString * ref_des, CString * pin_name, BOOL bSetAreas, BOOL bKeepConnections )
{
	...
	RemoveNetPin( net, net_pin, bSetAreas, 1, bKeepConnections );
}
...
// or the pin may not exist in the part 
//
void CNetList::RemoveNetPin( cnet * net, int net_pin_index, BOOL bSetAreas, BOOL bSetPartData, BOOL bKeepConnections )
{
	...
	while( ic<net->nconnects )
	{
		...
		else if( c->nsegs == 1 && c->seg[0].layer == LAY_RAT_LINE )
			RemoveNetConnect( net, ic, FALSE );
		else if( c->end_pin == net_pin_index )
		{
			if( c->seg[c->nsegs-1].layer == LAY_RAT_LINE && c->nsegs > 1 )
			{
				c->seg[c->nsegs-1].layer = c->seg[c->nsegs-2].layer;
				c->seg[c->nsegs-1].width = c->seg[c->nsegs-2].width;
			}
			ReverseNetConnect( net, ic, 0 );
			ic++;
		}
		else if( c->start_pin == net_pin_index && c->end_pin >= 0 )
		{
			if( c->seg[0].layer == LAY_RAT_LINE && c->nsegs > 1 )
			{
				c->seg[0].layer = c->seg[1].layer;
				c->seg[0].width = c->seg[1].width;
			}
			ReverseNetConnect( net, ic, 1 );
			DrawConnection( net, ic );
			ic++;
		}
		else if( c->start_pin == net_pin_index )
			RemoveNetConnect( net, ic, FALSE );
		else
			ic++;
	}
	...
	// adjust connections to areas
	if( net->nareas && bSetAreas && bSetPartData )
		SetAreaConnections( net );
}
...
int CNetList::GetLength( cnet * net, int  icon )
{
	int out = 0;
	for( int ic=0; ic<net->nconnects; ic++ )
	{
		if( icon == -1 || icon == ic )
			...
			{
				int x1 = net->connect[ic].vtx[is].x;
				int y1 = net->connect[ic].vtx[is].y;
				int x2 = net->connect[ic].vtx[is+1].x;
				int y2 = net->connect[ic].vtx[is+1].y;
...
BOOL CNetList::ReverseNetConnect( cnet * net, int ic, BOOL bRev )
{
	...
	if( bRev ) 
	{
		...
		for( int i=0; i<(nsegs+1)/2; i++ )
		{
			if( c->end_pin == cconnect::NO_END )
				ASSERT(0);
			if( c->vtx[c->nsegs-i].m_dlist == NULL )
				c->vtx[c->nsegs-i].m_dlist = m_dlist;
			// swap layer
			buf = c->seg[i].layer;
	...
	///if( bRev && c->seg[ c->nsegs-1 ].layer == LAY_RAT_LINE ) 
	///	RemoveSegment( net, ic, c->nsegs-1 );
	return TRUE;
}
...
int CNetList::GetNumRatlines( cnet * n, int * ratX, int * ratY, CString * Str, cpart ** P1, cpart ** P2, id * ID1,  id * ID2 )
{
	int bFORALL = !n;
	int nRat = 0;
	for( n=(n?n:GetFirstNet()); n; n=GetNextNet() )
	{
		for( int ic=0; ic<n->nconnects; ic++ )
			if( n->connect[ic].nsegs == 1 || bFORALL )
				if( n->connect[ic].end_pin >= 0 || bFORALL )
				{
					for( int iseg=0; iseg<n->connect[ic].nsegs; iseg++ )
					{
						if( n->connect[ic].seg[iseg].layer == LAY_RAT_LINE )
						{
							nRat++;
							if( ratX && ratY && Str && P1 && P2 && ID1 && ID2 && bFORALL == 0 )
							{
								*ratX = (n->connect[ic].vtx[0].x + n->connect[ic].vtx[1].x)/2.0;
								*ratY = (n->connect[ic].vtx[0].y + n->connect[ic].vtx[1].y)/2.0;
								if( n->pin[n->connect[ic].start_pin].part && n->pin[n->connect[ic].end_pin].part )
									Str->Format( "from %s.%s to %s.%s", n->pin[n->connect[ic].start_pin].part->ref_des,
																		n->pin[n->connect[ic].start_pin].pin_name,
																		n->pin[n->connect[ic].end_pin].part->ref_des,
																		n->pin[n->connect[ic].end_pin].pin_name );
								*P1 = n->pin[n->connect[ic].start_pin].part;
								*P2 = n->pin[n->connect[ic].end_pin].part;
								if( *P1 )
								{
									*ID1 = (*P1)->m_id;
									if( (*P1)->shape )
									{
										ID1->i = (*P1)->shape->GetPinIndexByName( n->pin[n->connect[ic].start_pin].pin_name, -1 );		
										ID1->st = ID_PAD;
									}
								}
								if( *P2 )
								{
									*ID2 = (*P2)->m_id;
									if( (*P2)->shape )
									{
										int pin2 = (*P2)->shape->GetPinIndexByName( n->pin[n->connect[ic].end_pin].pin_name, -1 );
										ID2->st = ID_PAD;
									}
								}
							}
...
//
int CNetList::ChangeSegmentLayer( cnet * net, int ic, int iseg, int layer, int vw, int vh, BOOL bReconcileVais )
{
...
int CNetList::DrawSegment( cnet * net, int ic, int is )
{
	...
	if( m_dlist )
	{
		...
		{
			if( net->connect[ic].locked )
				s->width = 3*NM_PER_MIL;
			else
				s->width = 0;
...
int CNetList::PartRouted( cpart * part )
{
	int CNT = 0;
	for( cnet * n = GetFirstNet(); n; n = GetNextNet(/*LABEL*/) )
	{
		...
		{
			if( n->connect[icon].nsegs < 1 )
				continue;
			if( n->pin[n->connect[icon].start_pin].part == part )
			{
				if( /*n->connect[icon].nsegs > 1 ||*/ n->connect[icon].seg[0].layer != LAY_RAT_LINE )
					CNT++;
			}
			else if( n->connect[icon].end_pin >= 0 )
			{
				if( n->pin[n->connect[icon].end_pin].part == part )
					if( /*n->connect[icon].nsegs > 1 ||*/ n->connect[icon].seg[n->connect[icon].nsegs-1].layer != LAY_RAT_LINE )
						CNT++;
			}
		}
	}
			...
			if( nsegs )
			{
				...
				if( net->pin[p2].part == part )
				{
					...
					if( pin_index2 == -1 )
					{
						...
						{
							// no, remove connection
							ReverseNetConnect( net, ic, FALSE );
							continue;
						}
						else 
						{
...
int CNetList::PartDeleted( cpart * part, BOOL bSetAreas )
{
	...
	for( pos = m_map.GetStartPosition(); pos != NULL; )
	{
		...
		for( int ip=0; ip<net->pin.GetSize(); )
		{
			if( net->pin[ip].ref_des == part->ref_des )
			{
				RemoveNetPin( net, &net->pin[ip].ref_des, &net->pin[ip].pin_name, FALSE, TRUE );
...
int CNetList::CancelDraggingInsertedAreaCorner( cnet * net, int iarea, int icorner )
{
	...
	if( m_dis_hatch )
		net->area[iarea].poly->SetHatchVisible(0);
	return 0;
}
...
void CNetList::SetAreaHatch( cnet * net, int iarea, int h )
{
	if( !net )
		return;
	net->area[iarea].poly->SetHatch( (h<0)?net->area[iarea].poly->GetHatch():h );
	if( m_dis_hatch )
		net->area[iarea].poly->SetHatchVisible(0);
}
...
void CNetList::HighlightRatLines( cnet * net, int ic, int swell )
{
	for( int icon=0; icon<net->nconnects; icon++ )
		for( int iseg=0; iseg<net->connect[icon].nsegs; iseg++ )
			if( net->connect[icon].seg[iseg].layer == LAY_RAT_LINE )
				HighlightSegment( net, icon, iseg, swell );
}
...
void CNetList::HighlightConnection( cnet * net, int ic, int is, BOOL W0, BOOL hv1, BOOL hv2, int transparent )
{
	...
	for(int iseg=0; iseg<c->nsegs; iseg++)
	{
		...
		if( !W0 )
		{
			...
			if( transparent )
				c->seg[iseg].dl_el->transparent = transparent;
		}
		else
		{
...
void CNetList::HighlightSegment( cnet * net, int ic, int iseg, int fClearance, int bTRANSPARENT, BOOL W0 )
{
	...
	if( c->seg[iseg].dl_el )
	{
		...
		if( bTRANSPARENT || W0 )
		{
			...
			//dl_element * sel = c->seg[iseg].dl_el;
			if( bTRANSPARENT )
...
void CNetList::HighlightNet( cnet * net, int bTRANSPARENT )
{
	...
	for( int ia=0;ia<net->nareas; ia++ )
	{
		...
		if( m_dis_hatch == 0 )
		{
			...
			HighlightAreaSides( net, ia, 1, TL );
	}
}
...
int CNetList::ReconcileVia( cnet * net, int ic, int ivtx, BOOL bDrawVia, int vw, int vh )
{
	...
	if( c->nsegs == 0 )
		return 0;
	// first vertex are part pads
	if( ivtx == 0 )
		return 0;
	// last vertex are part pads
	else if( c->end_pin != cconnect::NO_END && ivtx == c->nsegs )
		return 0;
	// see if via needed
	else if( v->force_via_flag && ivtx ) 
	{
		via_needed = TRUE;
	}
	// if branch
...
int CNetList::WriteNets( CStdioFile * file )
{
	...
	try
	{
		...
		for( pos = m_map.GetStartPosition(); pos != NULL; )
		{
			...
			for( int ia=0; ia<net->nareas; ia++ )
			{
				line.Format( "  area: %d %d %d %d %d %d %d %d\n", ia+1, 
					...
					net->area[ia].poly->GetMerge(TRUE),
		...
		else if( in_str.Left(4) == "net:" )
		{
						...
						if( test_not_done )
						{
							...
							if( end_pin != cconnect::NO_END )
							{
								if( is == (nsegs-1) )
								{
									...
									test_not_done = InsertSegment( net, ic, is, x, y, layer, seg_width, 0, 0, 0, 0 );
							}
							else
							{
			...
			for( int ia=0; ia<nareas; ia++ )
			{
				...
				if( np >= 6 )
					width = my_atoi( &p[4] );
				// test
				if( width && hatch == CPolyLine::DIAGONAL_EDGE )
					hatch = CPolyLine::NO_HATCH;
				if( width <= NM_PER_MIL && hatch == CPolyLine::DIAGONAL_FULL )
					width = 30000;
				// test end
				int merge = -1;
				if( np >= 7 )
				...
				int Submerge = -1;
				if( np >= 8 )
					Submerge = my_atoi( &p[6] );
				num_merges = max( num_merges, merge );
				num_merges = max( num_merges, Submerge );
				...
				for( int icor=0; icor<ncorners; icor++ )
				{
					...
					if( icor == 0 )
					{
						...
						net->area[ia].poly->SetMerge(Submerge, TRUE);
					}
					else
				...
				if( ia != (net->nareas-1) )
					ASSERT(0);
				if( net->area[ia].poly->GetNumCorners() != ncorners )
					ASSERT(0);
			}
...
int CNetList::DrawVia( cnet * net, int ic, int iv )
{
	...
	else
	{
		...
		if( iv == 0 )
		{
			lay = c->vtx[0].pad_layer;
			W = c->seg[0].width*2/5;
			if( c->nsegs )
				if( Distance( c->vtx[0].x, c->vtx[0].y, c->vtx[1].x, c->vtx[1].y ) < c->seg[0].width )
					W = c->seg[0].width/4;
		}
		else if( iv == c->nsegs && c->end_pin >= 0 )
		{
			lay = c->vtx[iv].pad_layer;
			W = c->seg[iv-1].width/4;
		}
		else if( iv < c->nsegs )
		{
			lay = c->seg[iv].layer;
			W = max( c->seg[iv].width, c->seg[iv-1].width );
			if( W < 2 )
				W = max( c->seg[iv].width, c->seg[iv-1].width );
			if( Distance( c->vtx[iv].x, c->vtx[iv].y, c->vtx[iv-1].x, c->vtx[iv-1].y ) < W || Distance( c->vtx[iv].x, c->vtx[iv].y, c->vtx[iv+1].x, c->vtx[iv+1].y ) < W )
				W /= 4;
			else
				W = W*2/5;
		}
		else
		{
			...
			if( Distance( c->vtx[iv].x, c->vtx[iv].y, c->vtx[iv-1].x, c->vtx[iv-1].y ) < c->seg[iv-1].width )
				W = c->seg[iv-1].width/4;
		}
								 ...
								 int def_w, int def_w_v, int def_w_v_h )
{
	...
	while( net )
	{
		...
		if( !bFound )
		{
			BOOL keep = !(flags & REM_NOT_NL_NETS);
	...
	while( net )
	{
		...
		for( int ia=net->nareas-1; ia>=0; ia-- )
		{
			...
			else
			{
				...
				poly->Draw();
				if( m_dis_hatch )
					net->area[ia].poly->SetHatchVisible(0); // 
			}
		}
...
void CNetList::RestoreConnectionsAndAreas( CNetList * old_nl, int flags, CDlgLog * log )
{
	...
	while( old_net )
	{
		if( flags & KEEP_AREAS )
		{
			...
			for( int ia=old_net->nareas-1; ia>=0; ia-- )
			{		
			...
			if( bMoveIt ) 
				if( old_net->name.CompareNoCase( new_area_net->name ) )
					for( int iarea=old_net->nareas-1; iarea>=0; iarea-- )
					{
						...
						int ian = AddArea( net, old_poly->GetLayer(), old_poly->GetX(0),
							old_poly->GetY(0), old_poly->GetHatch() );
						carea * a = &net->area[ian];
						a->poly->Copy( old_poly );
						id p_id( ID_NET, ID_AREA, ian, 0, 0 );
						a->poly->SetId( &p_id );
						a->poly->SetPtr( net );
						a->poly->SetMerge( old_poly->GetMerge() );
						a->poly->Draw( m_dlist );
						//
			...
			while( bRestored )
			{
				...
				for( int old_ic=0; old_ic<old_net->nconnects; old_ic++ )
				{
					...
					if( !new_start_net )
					{
						...
						{
							ReverseNetConnect( old_net, old_ic, TRUE );
							bKeepConnect = old_ic+1;
							old_ic--;
						}
					...
					{
						old_c->vtx[0].x = st_p.x;
						old_c->vtx[0].y = st_p.y;
					}
					...
					{
						if( new_start_net->name.Compare( old_net->name ) == 0 && bKeepConnect == 0 )
						{
							old_c->utility = 1;
					...
					else
					{
						...
						if( flags & KEEP_TRACES )
						{
							...
							//cpart * old_end_part = old_nl->m_plist->GetPart( old_end_pin->ref_des );
							...
							{
								ReverseNetConnect( old_net, old_ic, FALSE );
								bKeepConnect = old_ic+1;
								old_ic--;
								continue;
							}
							else if( new_start_net->name.Compare( old_net->name ) == 0 )
							{
								old_c->utility = 1;
								continue;	// ignore if start pin has not changed net
							}
							...
							{
								old_c->vtx[old_c->nsegs].x = e_p.x;
								old_c->vtx[old_c->nsegs].y = e_p.y;
							}
...
{	
	...
	if( max_cont >= 2 )
	{
		...
		for( int i=p->GetContourStart(nc); nc<(max_cont-1); i++ )
		{
			...
			{
				CString s;
				s.Format("checking %d from %d", nc, max_cont );
				pMain->DrawStatus( 3, &s );
			}
			//
			//
...
if( test == 1 )
	{
		...
		if( n_poly > 1 )
		{
			for( int ip=0; ip<pa->GetSize(); ip++ )
			{
				...
				net->area[ia].poly->FreeGpcPoly();
				SetAreaHatch( net, ia, hatch ); // include Draw();
				net->area[ia].utility = 1;
			}
			// when a polygon is complex, 
			// it can be automatically divided into several polygons 
			// and it happens that a cutout in a polygon refers 
			// to another polygon and not to which you need
			// dragging cutouts
			for( int sc_i=net->nareas-1; sc_i>=mem_n_areas; sc_i-- )
			{
				for( int ip=0; ip<pa->GetSize(); ip++ )
				{
					// create new copper area and copy poly into it
					CPolyLine * get_p = pa->GetAt(ip);
					int numc = get_p->GetNumContours();
					BOOL bRedraw = 0;
					for( int con=1; con<numc; con++ )
					{
						int st = get_p->GetContourStart(con);
						if ( net->area[sc_i].poly->TestPointInside( get_p->GetX(st),get_p->GetY(st) ))
						{
							bRedraw = 1;
							int end = get_p->GetContourEnd(con);
							for (int ic=st; ic<=end; ic++)
								net->area[sc_i].poly->AppendCorner(get_p->GetX(ic),get_p->GetY(ic),get_p->GetSideStyle(get_p->GetIndexCornerBack(ic)),FALSE);
							net->area[sc_i].poly->Close(get_p->GetSideStyle(end),FALSE);
						}
					}
					if( bRedraw )
						SetAreaHatch( net, sc_i ); // net->area[sc_i].poly->Draw();
				}
			}
		}
		delete pa;
...
int CNetList::CombineAreas( cnet * net, int ia1, int ia2 )
{
	...
	if( m_dis_hatch )
		net->area[ia1].poly->SetHatchVisible(0); // 
	gpc_free_polygon( union_gpc );
	delete union_gpc;
	return 1;
}
...
int CNetList::RemoveOrphanBranches( cnet * net, int id, BOOL CombineOrphan, int via, int hole )
{
	...
	else if ( n_tee_middle == 0 && n_tee_edge >= 2 && CombineOrphan)
	{
		...
		if( net->connect[ic2].start_pin != net->connect[ic1].start_pin || net->connect[ic2].start_pin_shape != net->connect[ic1].start_pin_shape )
		{
		...
		{
			net->connect[ic1].end_pin = cconnect::NO_END;
			net->connect[ic1].end_pin_shape = cconnect::NO_END;
			net->connect[ic1].vtx[net->connect[ic1].nsegs].pad_layer = 0;
		}
		// redraw

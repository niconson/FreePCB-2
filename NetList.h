...
public:
	...
	void RemoveNetPin( cnet * net, CString * ref_des, CString * pin_name, BOOL bSetAreas=TRUE, BOOL bKeepConnections=FALSE );
	void RemoveNetPin( cnet * net, int net_pin_index, BOOL bSetAreas=TRUE, BOOL bSetPartData=TRUE, BOOL bKeepConnections=FALSE );
	...
	void HighlightRatLines( cnet * net, int ic, int swell );
	...
	cnet * GetFirstNet();
	cnet * GetNextNet();
	void CancelNextNet( BOOL bRST = FALSE );
	int GetNumNets();
	...
	int GetLength( cnet * net, int  icon=-1 );
	...
	int GetNumRatlines( cnet * n=NULL, int * ratX=0, int * ratY=0, CString * Str=NULL, cpart ** P1=NULL, cpart ** P2=NULL, id * ID1=NULL,  id * ID2=NULL );
	...
	int ChangeSegmentLayer( cnet * net, int ic, int iseg, int layer, int vw, int vh, BOOL bReconcileVais = TRUE );							 
	...
	void SetAreaHatch( cnet * net, int iarea, int h=-1 );
	...
	void AddCutout(	CArray<gpc_polygon*> * Cutouts, 
					CArray<RECT> * Rects, 
					gpc_polygon ** gpc_union,
					gpc_polygon * gpc_tmp, 
					gpc_polygon * gpc_result,
					RECT * currR);
	int AddCutoutsForArea(	cnet * area_net, 
							int ia, 
							cnet * crop_net, 
							int pad_cl, 
							int seg_cl,
							...
							int cam_flags,  
							int selection_flags );
	int GenPolygonConnect( cnet * net, int icon, int layer, int clrnc, CArray<CPoint> * A );
	// I/O  functions

...
int CPartList::GetPinLayer( cpart * part, int pin_index )
{
	if( pin_index == -1 )
		return LAY_PAD_THRU;
	else if( part->shape->m_padstack[pin_index].hole_size )
		return LAY_PAD_THRU;
								  ...
								  CDisplayList * dlist, SMFontUtil * sm )
{
	...
	//
	CSize minmax = GetTextHeight( sm );
	double TH = minmax.cy - minmax.cx + 1.0;
	if( TH > 2.0 )
	{
		x_scale = (double)size/TH;
		y_scale = (double)size/TH;
		y_offset = -(double)minmax.cx*y_scale;
	}
	double sp_scale = (double)size/22.0; //2066.0*TH;
	double letter_spacing = sp_scale*3.5*sm->m_letter_spacing;
	//
	double xc = 0.0;
	CPoint si, sf;
	...
	for( int ic=0; ic<str->GetLength(); ic++ )
	{
		...
		int nstrokes = sm->GetCharStrokes( str->GetAt(ic), sm->m_font_number, 
		...
		xc += (max_x - min_x)*x_scale + w + letter_spacing;
	}
	if( mirror )
	{
		br->left = -br->right;
...
int CPartList::DrawPart( cpart * part )
{
	...
	if( npts )
	{
		...
		pid.st = ID_REF_TXT;
		pid.sst = ID_STROKE;
		part->dl_ref_el = m_dlist->Add( pid, part, lay_map, DL_LINES_ARRAY, 1, &br, part->m_ref_w, &LinesArray[0], npts );
	...
	if( npts )
	{
		...
		pid.st = ID_VALUE_TXT;
		pid.sst = ID_STROKE;
		part->dl_value_el = m_dlist->Add( pid, part, lay_map, DL_LINES_ARRAY, 1, &br, part->m_value_w, &LinesArray[0], npts );
	...
	dl_element * el_RctGr = NULL;
	{
		...
		pid.st = ID_OUTLINE;
		pid.sst = ID_STROKE;
		for( int step=0; step<7; step++ ) // First we write all the lines on top then on notes then on copper
		{
			int current_lay = 0;
			if( step == 0 )
			{
				if( !m_part_line_visible )
					continue;
				current_lay = LAY_SILK_TOP;
			}
			else if( step == 1 )
			{
				if( !m_part_line_visible )
					continue;
				current_lay = LAY_SILK_BOTTOM;
			}
			else if( step == 2 )
				current_lay = LAY_REFINE_TOP;
			else if( step == 3 )
				current_lay = LAY_REFINE_BOT;
			else if( step == 4 )
				current_lay = LAY_TOP_COPPER;
			else if( step == 5 )
				current_lay = LAY_BOTTOM_COPPER; 
			else if( step == 6 )
				current_lay = LAY_PAD_THRU;
			else
				ASSERT(0);
			// Set Layer
			lay_map = 0;	
			setbit(lay_map, current_lay);
			for( int ip=0; ip<p_sz; ip++ )
			{
				...
				// Get Layer
				int conv_lay = PCB_lay( poly->GetLayer() );
				int gside = Get_Side( conv_lay );
				if( part->side )
					if( gside == 0 )
						conv_lay++;
					else if( gside == 1 )
						conv_lay--;
				if( conv_lay != current_lay )
					continue;
				//
	...
	if( tsz )
	{
		...
		if( theApp )
			part->shape->m_tl->m_smfontutil = theApp->m_Doc->m_smfontutil;
	...
	for( int step=0; step<6; step++ )
	{
		...
		for( int it=0; it<tsz; it++ )
		{
			...
			int conv_lay = PCB_lay( t->m_layer );
			if( conv_lay == LAY_PAD_THRU )
				conv_lay = LAY_REFINE_BOT;
			int gside = Get_Side( conv_lay );
			if( part->side )
				if( gside == 0 )
					conv_lay++;
			...
			if(npts)
			{
				...
				part->m_outline_stroke[pos] = m_dlist->Add(	pid, part, lay_map, DL_LINES_ARRAY, 1, &br, w, PTS, npts );
				if( !el_RctGr )
					el_RctGr = part->m_outline_stroke[pos];
				else if( !el_magical )
					el_magical = part->m_outline_stroke[pos];
...
//
int CPartList::StartDraggingPart( CDC * pDC, cpart * part, int ipin, BOOL bRatlines, 
								 BOOL bBelowPinCount, int pin_count, BOOL bSaveStartPos )
{
	...
	if( ipin >= 0 )
	{
		startX = part->pin[ipin].x;
		startY = part->pin[ipin].y;
	}
	...
	if( psz == 0 )
	{
		...
		CPoint P[4];
		int np = 4;
		m_dlist->Get_Points( part->dl_sel, P, &np, -startX, -startY );
		if( np == 4 )
		{
		...
		else // rectangle
		{
			...
			CPoint pt[4];
			pt[0].x = Get.left		- startX;
			pt[0].y = Get.bottom	- startY;
			pt[1].x = Get.left		- startX;
			pt[1].y = Get.top		- startY;
			pt[2].x = Get.right		- startX;
			pt[2].y = Get.top		- startY;
			pt[3].x = Get.right		- startX;
			pt[3].y = Get.bottom	- startY;
	...
	else
	{
		...
		for( int ip=0; ip<part->shape->m_padstack.GetSize(); ip++ )
		{
			...
			CPoint P[4];
			int np = 4;
			m_dlist->Get_Points( part->pin[ip].dl_sel, P, &np, -startX, -startY );
			if( np == 4 )
			{
				for( int ii=0; ii<np-1; ii++ )
					m_dlist->AddDragLine( P[ii], P[ii+1] );
			...
			// save pin position
			pin_points[ip].x = part->pin[ip].x - startX;
			pin_points[ip].y = part->pin[ip].y - startY;
		}
	}
...
//**									if( !bBelowPinCount || n->npins <= pin_count )
						...
						if( pin2_part == part )
						{
							...
							if( ip != -1 )
							{
								if( c->seg[c->nsegs-1].dl_el->visible )
								{
...
//**									if( !bBelowPinCount || n->npins <= pin_count )
	...
	//
	m_dlist->StartDraggingArray( pDC, startX, startY );
	return 0;
}
...
int CPartList::ReadParts( CStdioFile * pcb_file )
{
	...
	while( 1 )
	{
		...
		else if( in_str.Left(4) == "ref:" || in_str.Left(5) == "part:" )
		{
			...
			cpart * nP = AddFromString( &str );
			if( nP )
			{
				max_num_merge = max( max_num_merge, nP->m_merge );
				max_num_merge = max( max_num_merge, nP->sub_merge );
			}
		}
	}
	return max_num_merge;
}
...
#undef REPEAT_REF
	...
	while( part->next != 0 )
	{
		...
		else if( !(flags & REM_PRES_PART_NETS) )
		{
			part->bPreserve = TRUE;
		}
		part = next_part;
				...
				if( old_part->shape )
				{
					...
					if( pi->shape )
					{
						if( pi->shape->m_package.Compare(old_part->shape->m_package) == 0 ||
							pi->shape->m_package.Compare("ERROR") == 0 )
						{
							if(pi->shape->m_name.Compare(old_part->shape->m_name) == 0 )
								sh_match = TRUE;
							if(flags & KEEP_FP_PACKAGE_MATCH)
								sh_match = TRUE;
						}
					}
					...
					if( (flags & KEEP_FP) || sh_match )
					{
						// keep footprint 
						if( log && pi->shape && old_part->shape )
						{
							if( old_part->shape->m_name.Compare( pi->shape->m_name ) )
							{
								mess.Format( "  Keeping footprint \"%s\" instead of the specified \"%s\" for part %s\r\n", old_part->shape->m_name, pi->shape->m_name, old_part->ref_des );
								log->AddLine( mess );
							}
						}
			...
			if( pi->part->shape != pi->shape || pi->bShapeChanged == TRUE )
			{
				...
				else 
				{
					...
					if(flags & KEEP_FP_PACKAGE_MATCH)
					{
						...
						{
							if( pi->shape->m_package.Compare( pi->part->shape->m_package ) )
								change_flag = TRUE;
						}
					}
					else if(!(flags & KEEP_FP))
						change_flag = TRUE;
					if( pi->shape && change_flag )
					{
						int prevx = pi->part->x;
						int prevy = pi->part->y;
...
cpart * CPartList::RefAutoLocation( cpart * p, int step, BOOL _90, RECT idsRect, CArray<CPolyLine> * bo, CTextList * tl, BOOL INCLUDE_HOLE, int Clearance  )
{
	...
	for ( int it=0; it<MAX_ITERATORS; it++ )
	{
		...
		if (pbad && p != pbad)
		{
			...
			///for( int ipo=0; ipo<pbad->m_outline_stroke.GetSize(); ipo++ )
			///{
			///	if( getbit( pbad->m_outline_stroke.GetAt(ipo)->layers_bitmap, LAY_PAD_THRU ) )
			///	{
			///		RECT get;
			///		m_dlist->Get_Rect( pbad->m_outline_stroke.GetAt(ipo), &get );
			///		intersection = max( RectsIntersection( pRef, get ), intersection );
			///	}
			///}
			// intersection
		...
		{
			if( pp->shape )
				if( pp->ref_des.Left(3) == "VIA" )
					if( pp->shape->GetNumPins() == 1 )
						if( pp->shape->m_padstack[0].top.shape != PAD_NONE )
							if( pp->shape->m_padstack[0].hole_size )
								pp->utility = 2; // skip vias
			...
			if( pp != p )
			{
				...
				///for( int ipo=0; ipo<pp->m_outline_stroke.GetSize(); ipo++ )
				///{
				///	if( getbit( pp->m_outline_stroke.GetAt(ipo)->layers_bitmap, LAY_PAD_THRU ) )
				///	{
				///		RECT get;
				///		m_dlist->Get_Rect( pp->m_outline_stroke.GetAt(ipo), &get );
				///		intersection = max( RectsIntersection( pRef, get ), intersection );
				///	}
				///}
				if (intersection >= 0)
...
void CPartList::ChannelDuplication( CString * Suffix, CArray<CPolyLine> * po, CTextList * tl )
{
	static int stat_shift = 1;
	CFrameWnd * pMainWnd = (CFrameWnd*)AfxGetMainWnd();
	CView * m_view = pMainWnd->GetActiveView();
	if( !m_view )
		return;
	// mark 0
	MarkAllParts(0);
	RECT R = m_dlist->GetHighlightedBounds(NULL);
	SwellRect( &R, NM_PER_MIL );
	int W = R.right - R.left + NM_PER_MM;
	int H = R.top - R.bottom;
	for( cpart * p1=GetFirstPart(); p1; p1=GetNextPart(p1) )
	{
		if( p1->selected )
		{
			CString pref1 = p1->ref_des;
			pref1.Truncate( p1->ref_des.GetLength() - Suffix->GetLength() );
			for( cpart * p2=GetFirstPart(); p2; p2=GetNextPart(p2) )
			{
				if( p2->selected )
					continue;
				if( p2->utility )
					continue;
				{
					CString pref2 = p2->ref_des;
					pref2.Truncate( p2->ref_des.GetLength() - Suffix->GetLength() );
					CString suff2 = p2->ref_des.Right( Suffix->GetLength() );
					if( pref1.Compare( pref2 ) == 0 )
					{
						if( suff2.Compare( *Suffix ) == 0 )
						{
							...
							{
								Move( p2, p1->x+W, p1->y, p1->angle, p1->side ); 
								CPoint ref_pt = GetRefPoint( p1 );
								MoveRefText( p2, ref_pt.x+W, ref_pt.y, p1->m_ref_angle, p1->m_ref_size, p1->m_ref_w );
								ref_pt = GetValuePoint( p1 );
								MoveValueText( p2, ref_pt.x+W, ref_pt.y, p1->m_value_angle, p1->m_value_size, p1->m_value_w );
								m_nlist->PartMoved( p2, TRUE );
								p2->utility = 1;
								CFreePcbApp * theApp = ((CFreePcbApp*)AfxGetApp());
								if( theApp )
								{
									theApp->m_Doc->m_view->OnRangeCmds( NULL );
									theApp->m_Doc->m_view->UpdateWindow();
								}
							}
						}
					}
	...
	for( cnet * scann=m_nlist->GetFirstNet(); scann; scann=m_nlist->GetNextNet() )
	{
		for( int icon=0; icon<scann->nconnects; icon++ )
		{
			if( scann->pin[scann->connect[icon].start_pin].part->selected )
			{
				BOOL OK = 0;
				if( scann->connect[icon].end_pin == cconnect::NO_END )
					OK = 1;
				else if( scann->pin[scann->connect[icon].end_pin].part->selected )
					OK = 1;
				if( OK )
				{
					cpart * p1 = scann->pin[scann->connect[icon].start_pin].part;
					cpart * p2 = NULL;
					CString pref1 = p1->ref_des;
					pref1.Truncate( p1->ref_des.GetLength() - Suffix->GetLength() );
					CString ref1 = pref1 + *Suffix;
					cpart * gp1 = GetPart( ref1 );
					cpart * gp2 = NULL;
					// add net connect
					if( gp1 )
					{
						if( gp1->selected )
							continue;
						if( p1->shape != gp1->shape )
							continue; // upps -(((
						cnet * newNet = NULL;
						if( scann->connect[icon].start_pin_shape >= 0 )
							if( scann->connect[icon].start_pin_shape < gp1->shape->GetNumPins() )
								if( gp1->pin[scann->connect[icon].start_pin_shape].net )
								{
									newNet = gp1->pin[scann->connect[icon].start_pin_shape].net;
								}
						if( scann->connect[icon].end_pin != cconnect::NO_END )
						{
							p2 = scann->pin[scann->connect[icon].end_pin].part;
							if( p2 )
							{
								CString pref2 = p2->ref_des;
								pref2.Truncate( p2->ref_des.GetLength() - Suffix->GetLength() );
								CString ref2 = pref2 + *Suffix;
								gp2 = GetPart( ref2 );
								if( gp2 )
								{
									if( gp2->selected )
										continue;
									if( p2->shape != gp2->shape )
										continue; // upps -(((
									BOOL bYES = 0;
									if( scann->connect[icon].end_pin_shape >= 0 )
										if( scann->connect[icon].end_pin_shape < gp2->shape->GetNumPins() )
											if( gp2->pin[scann->connect[icon].end_pin_shape].net )
												if( gp2->pin[scann->connect[icon].end_pin_shape].net == gp1->pin[scann->connect[icon].start_pin_shape].net )
													if( p2->shape == gp2->shape )
														bYES = 1;
									if( bYES == 0 )
										continue; // upps -(((
						...
						if( newNet )
						{
							CString pin_name1 = scann->pin[scann->connect[icon].start_pin].pin_name;
							int index1 = m_nlist->GetNetPinIndex( newNet, &gp1->ref_des, &pin_name1 );
							int index2 = cconnect::NO_END;
							if( gp2 )
							{
								CString pin_name2 = scann->pin[scann->connect[icon].end_pin].pin_name;
								index2 = m_nlist->GetNetPinIndex( newNet, &gp2->ref_des, &pin_name2 );
							}
							int new_ic;
							if( index2 >= 0 )
							{
								new_ic = m_nlist->AddNetConnect( newNet, index1, index2, scann->connect[icon].vtx[0].x+W,
																						 scann->connect[icon].vtx[0].y,
																						 scann->connect[icon].vtx[scann->connect[icon].nsegs].x+W,
																						 scann->connect[icon].vtx[scann->connect[icon].nsegs].y );
								newNet->connect[new_ic].seg[0].layer = scann->connect[icon].seg[scann->connect[icon].nsegs-1].layer;
								for( int i=0; i<scann->connect[icon].nsegs; i++ )
								{
									m_nlist->InsertSegment( newNet, new_ic, i, scann->connect[icon].vtx[i+1].x+W,
																			scann->connect[icon].vtx[i+1].y,
																			scann->connect[icon].seg[i].layer,
																			scann->connect[icon].seg[i].width,
																			scann->connect[icon].vtx[i+1].via_w,
																			scann->connect[icon].vtx[i+1].via_hole_w, 0 );
									if( scann->connect[icon].vtx[i].tee_ID )
									{
										int ID = scann->connect[icon].vtx[i].tee_ID + stat_shift;
										m_nlist->AddTeeID( ID );
										newNet->connect[new_ic].vtx[i].tee_ID = ID;
									}
									newNet->connect[new_ic].vtx[i+1].via_w = scann->connect[icon].vtx[i+1].via_w;
									newNet->connect[new_ic].vtx[i+1].via_hole_w = scann->connect[icon].vtx[i+1].via_hole_w;
								}
							}
							else 
							{
								new_ic = m_nlist->AddNetStub( newNet, index1, scann->connect[icon].vtx[0].x+W,
																						 scann->connect[icon].vtx[0].y );
								for( int i=0; i<scann->connect[icon].nsegs; i++ )
								{
									m_nlist->AppendSegment( newNet, new_ic, scann->connect[icon].vtx[i+1].x+W,
																			scann->connect[icon].vtx[i+1].y,
																			scann->connect[icon].seg[i].layer,
																			scann->connect[icon].seg[i].width,
																			scann->connect[icon].vtx[i+1].via_w,
																			scann->connect[icon].vtx[i+1].via_hole_w, TRUE );
									if( scann->connect[icon].vtx[i+1].tee_ID )
									{
										int ID = scann->connect[icon].vtx[i+1].tee_ID + stat_shift;
										m_nlist->AddTeeID( ID );
										newNet->connect[new_ic].vtx[i+1].tee_ID = ID;
									}
								}
							}
							m_nlist->DrawConnection( newNet, new_ic );
						}
					}
				}
			}
		}
		//if( scann->name.Left(3) == "NET" )
		//	continue;
		for( int ia=scann->nareas-1; ia>=0; ia-- )
		{
			if( scann->area[ia].selected == 0 )
				continue;
			///RECT aR = scann->area[ia].poly->GetBounds();
			///if( RectsIntersection( R, aR ) == 0 )
			{
				int new_ia = m_nlist->AddArea( scann, scann->area[ia].poly->GetLayer(),
										 scann->area[ia].poly->GetX(0)+W,
										 scann->area[ia].poly->GetY(0),
										 scann->area[ia].poly->GetHatch() );
				scann->area[new_ia].poly->SetW( scann->area[ia].poly->GetW() );
				for( int icr=1; icr<scann->area[ia].poly->GetNumCorners(); icr++ )
				{
					if( scann->area[ia].poly->GetNumContour(icr) != scann->area[ia].poly->GetNumContour(icr-1) )
						scann->area[new_ia].poly->Close( scann->area[ia].poly->GetSideStyle(icr-1), FALSE, FALSE );
					m_nlist->AppendAreaCorner( scann, new_ia, scann->area[ia].poly->GetX(icr)+W, 
															  scann->area[ia].poly->GetY(icr), 
															  scann->area[ia].poly->GetSideStyle(icr-1) );
				}
				m_nlist->CompleteArea( scann, new_ia, scann->area[ia].poly->GetSideStyle(scann->area[ia].poly->GetNumCorners()-1) );
			}
		}
	}
	for( int ipo=po->GetSize()-1; ipo>=0; ipo-- )
	{
		if( po->GetAt(ipo).ContourSelected(0) )
		{
			id ID = po->GetAt(ipo).GetId();
			int inew = po->GetSize();
			ID.i = inew;
			po->SetSize( inew+1 );
			po->GetAt(inew).SetDisplayList( m_dlist );
			po->GetAt(inew).Start(	po->GetAt(ipo).GetLayer(), 
									po->GetAt(ipo).GetW(), 
									NM_PER_MIL, 
									po->GetAt(ipo).GetX(0)+W,
									po->GetAt(ipo).GetY(0),
									po->GetAt(ipo).GetHatch(),
									&ID, NULL );
			for( int ico=1; ico<po->GetAt(ipo).GetNumCorners(); ico++ )
			{
				po->GetAt(inew).AppendCorner(	po->GetAt(ipo).GetX(ico)+W,
												po->GetAt(ipo).GetY(ico),
												po->GetAt(ipo).GetSideStyle(ico-1) );
				if( po->GetAt(ipo).GetContourEnd( po->GetAt(ipo).GetContour( ico ) ) == ico )
					po->GetAt(inew).Close( po->GetAt(ipo).GetNumCorners() );
			}
...
CPoint CPartList::AlignByRatline( cpart * PART, int i, BOOL Validate )
{
	...
	if( pad_net && theApp )
	{
		if( theApp->m_Doc->m_vis[LAY_RAT_LINE] == 0 )
			return CPoint(0,0);
		if( pad_net->visible == 0 )
			return CPoint(0,0);
		//
		RECT partR;
		GetPartBoundingRect( PART, &partR );
		CRect r = partR;
		int min_size = min( r.Width(), abs(r.Height()) );
		//if( abs( theApp->m_Doc->m_part_grid_spacing ) > (min_size/3) )
		//	return CPoint(0,0);
		int Xmin_len = INT_MAX;
		int Ymin_len = INT_MAX;
		CPoint pos1( PART->pin[i].x, PART->pin[i].y );
		int dx = INT_MAX;
		int dy = INT_MAX;
		for( int conn=0; conn<pad_net->nconnects; conn++ )
		{
			if( pad_net->connect[conn].nsegs < 1 )
				continue;
			if( pad_net->connect[conn].m_selected )
				continue;
			if( pad_net->pin[ pad_net->connect[conn].start_pin ].part == PART )
				if( pad_net->connect[conn].start_pin_shape == i )
					if( pad_net->connect[conn].seg[0].layer == LAY_RAT_LINE )
					{
						BOOL ok = 0;
						if( pad_net->connect[conn].end_pin == cconnect::NO_END )
							ok = 1;
						else if( pad_net->pin[ pad_net->connect[conn].end_pin ].part )
						{
							if( pad_net->pin[ pad_net->connect[conn].end_pin ].part->selected == 0 )
								ok = 1;
						}
						if( ok )
						{
							int dis = Distance(	pad_net->connect[conn].vtx[0].x, pad_net->connect[conn].vtx[0].y,
												pad_net->connect[conn].vtx[1].x, pad_net->connect[conn].vtx[1].y );
			...
			if( pad_net->connect[conn].end_pin == cconnect::NO_END )
				continue;
			if( pad_net->pin[ pad_net->connect[conn].end_pin ].part == PART )
				if( pad_net->connect[conn].end_pin_shape == i )
					if( pad_net->connect[conn].seg[pad_net->connect[conn].nsegs-1].layer == LAY_RAT_LINE )
					{
						if( pad_net->pin[ pad_net->connect[conn].start_pin ].part )
							if( pad_net->pin[ pad_net->connect[conn].start_pin ].part->selected == 0 )
							{
								int dis = Distance(	pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs].x, 
													pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs].y,
													pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs-1].x, 
													pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs-1].y );
								BOOL VERTICAL = (abs( pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs].x - 
													  pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs-1].x ) < 
												 abs( pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs].y - 
													  pad_net->connect[conn].vtx[pad_net->connect[conn].nsegs-1].y ));
		...
		if( dx < INT_MAX || dy < INT_MAX )
		{
			...
			if( Validate == FALSE )
			{
				theApp->m_Doc->m_plist->Move(	PART, PART->x+dx, PART->y+dy, PART->angle, PART->side );
				theApp->m_Doc->m_nlist->PartMoved( PART , TRUE );
			}
			return CPoint(dx,dy);
		}	
	}

...
void CPolyLine::Draw(  CDisplayList * dl )
{
	...
	m_nhatch = 0;
	if( m_dlist && m_sel_box && m_ncorners )
	{
		dl_element * el_magical = NULL;
		dl_element * el_rect_magic = NULL;

		BOOL bOpenSCAD = (m_openscad_data.Find("_extrude")>0);
		...
		for( int ic=0; ic<m_ncorners; ic++ )
		{
			...
			if( ds )
			{
				...
				if( abs(m_w) <= m_pcbu_per_wu )
					SwellRect( &r, m_pcbu_per_wu*100);
				else
				...
				else if( !el_rect_magic )
					el_rect_magic = dl_side[ic];
				if( m_visible == 0 )
					dl_side[ic]->transparent = TRANSPARENT_BACKGND;
			}
			// draw corner
			m_id.sst = ID_CORNER;
			int p_w = abs(m_w)/2;
			...
			r.top =    P[0].y + p_w;
			r.bottom = P[0].y - p_w;
			int typ = (bOpenSCAD && ic == 0)?DL_HOLLOW_CIRC:DL_CIRC;
			dl_corner_sel[ic] = m_dlist->AddSelector( m_id, m_ptr, typ, 1, 
				&r, 0, NULL, 0, lay_map );
			if( bOpenSCAD && ic == 0 )
				setbit( dl_corner_sel[ic]->layers_bitmap, m_layer );
		}
		if( el_magical )
		...
		//
		if( el_rect_magic )
			if( el_rect_magic->next != m_dlist->Get_End() )
			{
				RECT gB = GetBounds();
				SwellRect( &gB, NM_PER_MIL*5 );
				el_rect_magic->magic = m_dlist->Get_End()->prev;
				m_dlist->Set_RectGroup( el_rect_magic, gB.left, gB.right, gB.bottom, gB.top );
			}
		bDrawn = TRUE;
	}
...
void CPolyLine::StartDraggingToMoveCorner( CDC * pDC, int ic, int x, int y, int crosshair )
{
		...
		&& (ic == istart || ic == iend) )
	{
		...
		if( ic == istart )
			dl_corner_sel[iside]->visible = 0;
		else if( ic == iend )
			dl_corner_sel[iside+1]->visible = 0;
...
dl_element * CPolyLine::GetHatchLoc( int index )
{
	return dl_hatch[index];
}
...
{
	for( int n=0; n<select_s.GetSize(); n++ )
	{
		if( select_s[n] )
			return 1;
	}
...
int CPolyLine::ContourSelected( int nC )
{
	int C = 0;
	int s = cont_data[nC].start_c;
	int e = cont_data[nC].end_c;
	if( e == -1 )
		e = m_ncorners-2;
	for( int i=s; i<=e; i++ )
		if( select_s[i] )
...
{	
	if( m_dlist )
	{
		...
		for( int is=0; is<ns; is++ )
			dl_side[is]->visible = visible; 
		for( int is=0; is<m_ncorners; is++ )
			dl_corner_sel[is]->visible = visible;
		for( int ih=0; ih<m_nhatch; ih++ )
...
int CPolyLine::GetNumArcs( int n_cont )
{
	int n_a = 0;
	for( int i=0; i<GetNumSides(); i++ )
	{
		if( n_cont == GetNumContour(i) || n_cont == -1 )
...
void CPolyLine::Hatch()
{
	...
	if( m_dlist )
	{
		if( m_hatch == NO_HATCH )
			return;
		else if( m_id.st == ID_BOARD )
		{
			int lay_map = 0;
			setbit( lay_map, m_layer );
			int numC = GetNumContours();
			dl_hatch.SetSize( numC );
			for( int n=0; n<numC; n++ )
			{
				RECT r;
				r.left = r.bottom = INT_MAX;
				r.right = r.top = INT_MIN;
				int start = GetContourStart(n);
				int end = GetContourEnd(n);
				int nArcs = GetNumArcs(n);
				int numPTS = (end-start+1) + (nArcs*N_SIDES_APPROX_ARC);
				CPoint * pts = new CPoint[numPTS];
				int i = 0;
				for( int nCor=start; nCor<=end; nCor++ )
				{
					SwellRect( &r, corner[nCor].x, corner[nCor].y );
					if( GetSideStyle(nCor) == CPolyLine::STRAIGHT )
					{
						pts[i].x = corner[nCor].x;
						pts[i].y = corner[nCor].y;
						i++;
					}
					else
					{
						int ib = GetIndexCornerNext( nCor );
						int xb = corner[ib].x;
						int yb = corner[ib].y;
						int num_corners = Generate_Arc( corner[nCor].x,
														corner[nCor].y,
														xb, yb, GetSideStyle(nCor), &(pts[i]), N_SIDES_APPROX_ARC-1 );
						if( num_corners != N_SIDES_APPROX_ARC )
						{
							pts[i].x = corner[nCor].x;
							pts[i].y = corner[nCor].y;
							i++;
							...
							i += N_SIDES_APPROX_ARC;
					}	
				}
				id hatch_id = m_id;
				if( n )
					hatch_id.sst = ID_HOLE;
				else
					hatch_id.sst = ID_HATCH;
				hatch_id.ii = n;	
				dl_element * dl = m_dlist->Add( hatch_id, 0, lay_map, DL_POLYGON, 1, &r, 0, pts, i );
				dl_hatch.SetAtGrow(n, dl);
				delete pts;
			}
			m_nhatch = dl_hatch.GetSize();
		}	
		else if( GetClosed() )
		{
			enum {
			...
			for( int a=min_a; a<max_a; a+=spacing )
			{
				...
				{
					CString s;
					s.Format( "Hatch %d from %d", (a-min_a)/NM_PER_MM, (max_a-min_a)/NM_PER_MM ); 
					pMain->DrawStatus( 3, &s );
				}
				...
				{	
					if( m_hatch == DIAGONAL_FULL )
						...
						{
							hatch_id.ii = nhatch;
							dl_element * dl = m_dlist->Add( hatch_id, 0, lay_map, DL_LINES_ARRAY, 1, &r, wid, pts, 20 );
					...
					if( abs(dx) <= wid/2 )//|| abs(dy) <= min_dxy )
						continue;
			...
			if( m_hatch == DIAGONAL_FULL )
				if( caret )
				{
					...
					hatch_id.sst = ID_HATCH;
					hatch_id.ii = nhatch;
					dl_element * dl = m_dlist->Add( hatch_id, 0, lay_map, DL_LINES_ARRAY, 1, &r, wid, pts, caret );
					dl_hatch.SetAtGrow(nhatch, dl);
					caret = 0;
					nhatch++;
				}	
...
{
	if( sel_box < (NM_PER_MIL) )
		m_sel_box = NM_PER_MIL*10;
	else
		m_sel_box = sel_box;
}
...
void CPolyLine::Copy( CPolyLine * src )
{
	...
	m_openscad_data = src->m_openscad_data;
...
void CPolyLine::SetW( int w ) { m_w = w; SetSelBoxSize( abs(m_w) ); }
...
CString * CPolyLine::GetOpenscadData()
{
	return &m_openscad_data;
}

void CPolyLine::SetOpenscadData( CString * descriptor )
{
	m_openscad_data = *descriptor;
}

CString CPolyLine::GetOpenscadPolyPts( int units, int Fn, int tab, int centroid_x, int centroid_y, int start )
{
	static int cur = 0;
	if( start != -1 )
		cur = start;
	...
	CString S="", str;
	int prev_cont = corner[cur].num_contour;
	int i=cur;
	for( ; i<m_ncorners; i++ )
	{
		if( corner[i].num_contour != prev_cont )
			break;
		double x = corner[i].x;
		double y = corner[i].y;
		double x2 = corner[GetIndexCornerNext(i)].x;
		double y2 = corner[GetIndexCornerNext(i)].y;
...
void CPolyLine::GetOpenscadCube(	int *minx, 
									int *miny, 
									int *minz, 
									int *maxx, 
									int *maxy, 
									...
									int bMinkowski )
{
	...
	int num_corners = m_ncorners;
	if(extrude == 0)
		num_corners = 4;

	CPoint * PTS = new CPoint[num_corners *2];
	int * int_pts = new int[num_corners *6];
	
	if (extrude == 0)
		num_corners = 1;
	for( int i=0; i<num_corners; i++ )
	{
		int_pts[i*6]   = corner[i].x - cx;
		int_pts[i*6+1] = corner[i].y - cy;
		int_pts[i*6+2] = 0;
		int_pts[i*6+3] = corner[i].x - cx;
		int_pts[i*6+4] = corner[i].y - cy;
		int_pts[i*6+5] = extrude;
	}
	if (extrude == 0)
	{
		...
		for (int i = 0; i < m_ncorners; i++)
		{
			int gx = corner[i].x - cx;
			int gy = corner[i].y - cy;
			fminx = min(fminx, gx);
			fminy = min(fminy, gy);
			fmaxx = max(fmaxx, gx);
			fmaxy = max(fmaxy, gy);
		...
		*minx = int_pts[0] - ddx;
		*miny = int_pts[1] - ddx;
		*minz = fminy - int_pts[1];
		*maxx = int_pts[3] + ddx;
		*maxy = int_pts[4] + ddx;
		*maxz = fmaxy - int_pts[1];
		num_corners = 4;
		//
		int_pts[0] = *minx;
		int_pts[1] = *miny;
	...
	{	// x = -z
		// y = y
		PTS[i*2].y = int_pts[i*6+1];
		PTS[i*2].x = -int_pts[i*6+2];	
		PTS[i*2+1].y = int_pts[i*6+4];
		PTS[i*2+1].x = -int_pts[i*6+5];
	}
	RotatePOINTS( PTS, 2* num_corners, rotateX, (Zero) );
	for( int i=0; i< num_corners; i++ )
	{
		//int_pts[i*6] = ?
		int_pts[i*6+1] = PTS[i*2].y;
		int_pts[i*6+2] = -PTS[i*2].x;
		//int_pts[i*6+3] = ?
		int_pts[i*6+4] = PTS[i*2+1].y;
		int_pts[i*6+5] = -PTS[i*2+1].x;
	}
	//YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY
	for( int i=0; i< num_corners; i++ )
	{	// x = x
		// y = -z
		PTS[i*2].x = int_pts[i*6];
		PTS[i*2].y = -int_pts[i*6+2];
		PTS[i*2+1].x = int_pts[i*6+3];
		PTS[i*2+1].y = -int_pts[i*6+5];
	}
	RotatePOINTS( PTS, 2* num_corners, rotateY, (Zero) );
	for( int i=0; i< num_corners; i++ )
	{
		int_pts[i*6] = PTS[i*2].x;
		//int_pts[i*6+1] = ?
		int_pts[i*6+2] = -PTS[i*2].y;
		int_pts[i*6+3] = PTS[i*2+1].x;
		//int_pts[i*6+4] = ?
		int_pts[i*6+5] = -PTS[i*2+1].y;
	}
	...
	RotatePOINTS( PTS, 2* num_corners, rotateZ, (Zero) );
	for( int i=0; i< num_corners; i++ )
	{
		int_pts[i*6] = PTS[i*2].x;
		int_pts[i*6+1] = PTS[i*2].y;
	...
	for( int i=0; i< num_corners; i++ )
	{
		int_pts[i*6] +=   translateX + cx;
		int_pts[i*6+1] += translateY + cy;
		int_pts[i*6+2] += translateZ;
		int_pts[i*6+3] += translateX + cx;
		int_pts[i*6+4] += translateY + cy;
		int_pts[i*6+5] += translateZ;
	}
	//--------------------------------------------------------
	...
	*maxx = *maxy = *maxz = INT_MIN;
	for( int i=0; i< num_corners; i++ )
	{
		*minx = min( *minx, int_pts[i*6] );
		*minx = min( *minx, int_pts[i*6+3] );
		*miny = min( *miny, int_pts[i*6+1] );
		*miny = min( *miny, int_pts[i*6+4] );
		*minz = min( *minz, int_pts[i*6+2] );
		*minz = min( *minz, int_pts[i*6+5] );
		*maxx = max( *maxx, int_pts[i*6] );
		*maxx = max( *maxx, int_pts[i*6+3] );
		*maxy = max( *maxy, int_pts[i*6+1] );
		*maxy = max( *maxy, int_pts[i*6+4] );
		*maxz = max( *maxz, int_pts[i*6+2] );
		*maxz = max( *maxz, int_pts[i*6+5] );
	}
	if( stepX > 0 )
		*maxx += (matrixX-1)*stepX;
	else
		*minx += (matrixX-1)*stepX;
	if( stepY > 0 )
		*maxy += (matrixY-1)*stepY;
	else
		*miny += (matrixY-1)*stepY;
	if( stepZ > 0 )
		*maxz += (matrixZ-1)*stepZ;
	else
		*minz += (matrixZ-1)*stepZ;

...
public:
	...
	int ContourSelected( int nC );
	int GetMerge(BOOL bSUB=0);
	int GetNumCorners();
	int GetNumSides();
	int GetNumArcs( int n_cont=-1 );
	int GetClosed();
	int GetNumContours();
	...
	dl_element * GetHatchLoc( int index );
	CDisplayList * GetDisplayList(){ return m_dlist; };
	CString * GetOpenscadData();
	CString   GetOpenscadPolyPts( int units, int Fn, int tab, int centroid_x, int centroid_y, int start=-1 );
	void GetOpenscadCube(	int *minx, 
							int *miny, 
							int *minz, 
							int *maxx, 
							int *maxy, 
	...
	void SetOpenscadData( CString * descriptor );

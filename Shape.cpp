...
void CShape::Clear()
{
	m_openscad_code.RemoveAll();
	m_openscad_hole.RemoveAll();
	m_openscad_module.RemoveAll();
	m_name = EMPTY_SHAPE;
	m_author = "";
	m_package = "";
	m_desc = "";
						 ...
						 CString file_path, int pos )
{
	...
	try
	{
		...
		m_name = p[0].Left(MAX_NAME_SIZE).Trim();
		m_package = "";
		...
		while( 1 )
		{
			...
			else if( key_str == "openscad_code" && np >= 2 )
			{
				if( p[0].GetLength() )
					m_openscad_code.Add( p[0] );
			}
			else if (key_str == "openscad_hole" && np >= 2)
			{
				if (p[0].GetLength())
					m_openscad_hole.Add(p[0]);
			}
			else if (key_str == "openscad_module" && np >= 2)
			{
				if (p[0].GetLength())
					m_openscad_module.Add(p[0]);
			}
			...
			else if( key_str == "text" && np >= 7 )
			{
				...
				if( np >= 9 )
				{
					...
					// fix layer
					if( layer == LAY_FP_BOTTOM_MASK )
						layer = LAY_FP_TOP_COPPER;
				}
				if( np >= 10 )
				...
				&& np >= 4 )
			{
				...
				// fix error
				if( glayer == LAY_FP_BOTTOM_MASK )
					glayer = LAY_FP_TOP_COPPER;
			...
			else if( key_str == "openscad_data" && np >= 2 )
			{
				int npolys = m_outline_poly.GetSize();
				m_outline_poly[npolys-1].SetOpenscadData( &p[0] );
			}
...
int CShape::Copy( CShape * shape )
{
	...
	m_openscad_code.SetSize(0);
	for(int i=0; i<shape->m_openscad_code.GetSize(); i++)
		m_openscad_code.Add( shape->m_openscad_code.GetAt(i) );
	m_openscad_hole.SetSize(0);
	for (int i = 0; i < shape->m_openscad_hole.GetSize(); i++)
		m_openscad_hole.Add(shape->m_openscad_hole.GetAt(i));
	m_openscad_module.SetSize(0);
	for (int i = 0; i < shape->m_openscad_module.GetSize(); i++)
		m_openscad_module.Add(shape->m_openscad_module.GetAt(i));
...
BOOL CShape::Compare( CShape * shape )
{
	...
	//if( m_openscad_code != shape->m_openscad_code )
	//	return FALSE;
	return TRUE;
}

int CShape::Move3dObject( CArray<CString>* Astr, int dx, int dy, int units )
{
	if(Astr->GetSize() < 5 )
		return 0;
	CString s = Astr->GetAt(0);
	if( s.Find( "(automatic" ) == -1 )
		return 0;
	...
	s = Astr->GetAt(0);
	int icomm = s.Find( "//" );
	CString Comment = s.Right( s.GetLength() - icomm );
	icomm = 0;
	CString str = s.Tokenize("])", icomm);
	str.Replace( "([", " " );
	str.Replace( ",", " " );
	CArray<CString> arr;
	CString key;
	...
	y -= dy;
	CString xstr, ystr;
	::MakeCStringFromDimension( &xstr, x, units, 0, 0, 0, 3, 0 );
	::MakeCStringFromDimension( &ystr, y, units, 0, 0, 0, 3, 0 );
	str.Format("translate([%s,%s,%.3f]) %s", xstr, ystr, z, Comment );
	Astr->SetAt( 0, str );
	return 0;
}

int CShape::Scale3dObject( CArray<CString>* Astr, double scale )
{
	if( Astr->GetSize() < 5 )
		return 1;
	CString s = Astr->GetAt(0);
	int icomm = s.Find( "//" );
	CString Comment1 = s.Right( s.GetLength() - icomm );
	if( s.Find( "(automatic" ) == -1 )
		return 1;
	if( s.Find( "translate" ) == -1 )
		return 1;
	...
	s = Astr->GetAt(1);
	icomm = s.Find( "//" );
	CString Comment2 = s.Right( s.GetLength() - icomm );
	if( s.Find( "(automatic" ) == -1 )
		return 1;
	if( s.Find( "scale" ) == -1 )
		return 1;
	...
	str = arr[0];
	double x = my_atof( &str );
	str = arr[1];
	double y = my_atof( &str );
	str = arr[2];
	double z = my_atof( &str );
	x *= scale;
	y *= scale;
	z *= scale;
	str.Format("translate([%.3f,%.3f,%.3f]) %s", x, y, z, Comment1 );
	Astr->SetAt( 0, str );

	s = Astr->GetAt(1);
	icomm = 0;
	str = s.Tokenize("])", icomm);
	str.Replace( "([", " " );
	str.Replace( ",", " " );
	np = ParseKeyString( &str, &key, &arr );
	if( np < 4 )
		return 1;
	...
	y *= scale;
	z *= scale;
	str.Format("scale([%.3f,%.3f,%.3f]) %s", x, y, z, Comment2 );
	Astr->SetAt( 1, str );

	return 0;
}

int CShape::GetNumPins( int maxDifferent )
{
	if( maxDifferent )
	{
		CString PINS = " ";
		...
		for( int ip=0; ip<m_padstack.GetSize(); ip++ )
		{
			CString str = ( " " + m_padstack[ip].name + " " );
			if( PINS.Find( str ) <= 0 )
			{
				PINS += str;
				CNT++;
...
int CShape::WriteFootprint( CStdioFile * file )
{
	...
	try
	{
		...
		file->WriteString( line );
		///////////////////////////////////////////////////////
		for( int icode=0; icode<m_openscad_code.GetSize(); icode++ )
		{
			line.Format( "  openscad_code: \"%s\"\n", m_openscad_code.GetAt(icode) );
			file->WriteString( line );
		}
		for (int icode = 0; icode < m_openscad_hole.GetSize(); icode++)
		{
			line.Format("  openscad_hole: \"%s\"\n", m_openscad_hole.GetAt(icode));
			file->WriteString(line);
		}
		for (int icode = 0; icode < m_openscad_module.GetSize(); icode++)
		{
			line.Format("  openscad_module: \"%s\"\n", m_openscad_module.GetAt(icode));
			file->WriteString(line);
		}
		///////////////////////////////////////////////////////
...
HENHMETAFILE CShape::CreateMetafile( CMetaFileDC * mfDC, CDC * pDC, int x_size, int y_size )
{
	...
	for( int it=0; it<m_tl->text_ptr.GetSize(); it++ )
	{
		...
		// from Schematic Constructor
		CSize minmax = GetTextHeight( smfontutil );
		double TH = minmax.cy - minmax.cx + 1.0;
		if( TH > 2.0 )
		{
			x_scale = (double)t->m_font_size/TH;
			y_scale = (double)t->m_font_size/TH;
		...
		for( int ic=0; ic<t_str.GetLength(); ic++ )
		{
			...
			int nstrokes = smfontutil->GetCharStrokes( t_str[ic], smfontutil->m_font_number, 
	...
	for( int ic=0; ic<ref.GetLength(); ic++ )
	{
		...
		double coord[64][4];
		double min_x, min_y, max_x, max_y;
		int nstrokes = smfontutil->GetCharStrokes( ref[ic], smfontutil->m_font_number, 
			&min_x, &min_y, &max_x, &max_y, coord, 64 );
		for( int is=0; is<nstrokes; is++ )
		{
			// draw each stroke
	...
	*/
	...
	mfDC->MoveTo( x_min+xoffset, -y_min+yoffset );
	mfDC->LineTo( x_max+xoffset, -y_min+yoffset );
	mfDC->LineTo( x_max+xoffset, -y_max+yoffset );
	mfDC->LineTo( x_min+xoffset, -y_max+yoffset );
	mfDC->LineTo( x_min+xoffset, -y_min+yoffset );
...
RECT CShape::GetAllPadBounds()
{
	...
	if (!allpads) for( int ip=0; ip<m_outline_poly.GetSize(); ip++ )
	{
		RECT pr = m_outline_poly.GetAt(ip).GetCornerBounds(0);
		r.left = min( r.left, pr.left );
		r.bottom = min( r.bottom, pr.bottom );
...
#define NUM_L	7
				...
				if( ps->angle%90 || p->shape == PAD_OCTAGON )
				{
					...
					else
					{
						...
						if( ps->angle )
							RotateRect(&r, -ps->angle, pin);
						if( dlist )
							*pad_el = dlist->Add(	p_id, NULL, lay_map, gtype, 1, &r, 0, pts, npts+1 );
					}
				}
				else
				{
	...
	// from Schematic Constructor
	CSize minmax = GetTextHeight( fontutil );
	double TH = minmax.cy - minmax.cx + 1.0;
	if( TH > 2.0 )
	{
		x_scale = (double)m_ref_size/TH;
		y_scale = (double)m_ref_size/TH;
		y_offset = -(double)minmax.cx*y_scale;
	}
	...
	for( int ic=0; ic<3; ic++ )
	{
		...
		double coord[64][4];
		double min_x, min_y, max_x, max_y;
		int nstrokes = fontutil->GetCharStrokes( ref_str[ic], fontutil->m_font_number, 
			&min_x, &min_y, &max_x, &max_y, coord, 64 );
	...
	if( m_value_size )
	{
		...
		if( TH > 2.0 )
		{
			x_scale = (double)m_value_size/TH;
			y_scale = (double)m_value_size/TH;
			y_offset = -(double)minmax.cx*y_scale;
		}
		...
		for( int ic=0; ic<5; ic++ )
		{
			...
			double coord[64][4];
			double min_x, min_y, max_x, max_y;
			int nstrokes = fontutil->GetCharStrokes( value_str[ic], fontutil->m_font_number, 
				&min_x, &min_y, &max_x, &max_y, coord, 64 );
...
BOOL CShape::SyncOpenscadGlobalParameters( CString * g_rot, CString * g_lift, int * bHULL  )
{
	...
	for( int i=0; i<m_outline_poly.GetSize(); i++ )
	{
		CString * s = m_outline_poly.GetAt(i).GetOpenscadData();

		///if( s->GetLength() == 0 ) !!!
		///	continue;

		int f = s->Find( dlg.GROT );
		if( f > 2 )
		{
			int f2 = s->Find( sep, f );
			str = s->Mid( f-3, f2-f+4 );
			CArray<CString> arr;
			int np = ParseKeyString( &str, &key, &arr );
			if( g_rot->GetLength() == 0 )
			{
				if( np > 3 )
					*g_rot = arr[2];
				else
			...
			{
				CString new_str;
				new_str.Format( "// %s %s //%s", dlg.GROT, *g_rot, sep );
				s->Replace( str, "" );
				*s = new_str + *s;
			}
		}
		else if( g_rot->GetLength() == 0 )
			*g_rot = "0.0";
		else if( F1 == 0 )
		{
			CString new_str;
		...
		if( f > 2 )
		{
			...
			CArray<CString> arr;
			int np = ParseKeyString( &str, &key, &arr );
			if( g_lift->GetLength() == 0 )
			{
				if( np > 3 )
					*g_lift = arr[2];
				else
					*g_lift = (doc->m_units==MM?"0.0 mm":"0.0 mil");
				F2 =TRUE;
			}
			else if( F2 == 0 )
			{
				CString new_str;
				new_str.Format( "// %s %s //%s", dlg.GLIFT, *g_lift, sep );
				s->Replace( str, "" );
				*s = new_str + *s;
			}
		}
		else if( g_lift->GetLength() == 0 )
			*g_lift = (doc->m_units==MM?"0.0 mm":"0.0 mil");
		else if( F2 == 0 )
		{
			CString new_str;
		...
		if( bHULL )
		{
			int fHull = s->Find( dlg.HULL );
			if( fHull > 0 && *bHULL > 0 )
			{
			}
			else if( fHull == -1 && *bHULL == 0 )
			{
			}
			else
			{
				CString new_str;
				new_str.Format( "// %s //%s", dlg.HULL, sep );
				if( *bHULL > 0 )
					*s = new_str + *s;
				else
					s->Replace( new_str, "" );
...
CString CShape::GenerateOpenscadFileA( CString * fileName, BOOL bPreview )
{
	CFreePcbDoc * doc = ((CFreePcbApp*)AfxGetApp())->m_Doc;
	CDlg_OpenScad dlg;
	...
	if( ok )
	{
		CString global_r="", global_z="";
		BOOL bHull = SyncOpenscadGlobalParameters( &global_r, &global_z );
		...
		for( int i=0; i<m_outline_poly.GetSize(); i++ )
		{
			...
			CString * s = m_outline_poly.GetAt(i).GetOpenscadData();
			dlg.Initialize( s, &global_r, &global_z, bHull );

			// обновим строку т к ед. изм. могли поменяться
			// обновим но не будем сохранять
			s = dlg.GenerateOpenscadData( &sep );

			// save temporary data
			t_OpenScadData.Add( *s );

			if( s->GetLength() == 0 )
				continue;
			int mix=0, miy=0, miz=0, max=0, may=0, maz=0;
			m_outline_poly.GetAt(i).GetOpenscadCube(		&mix, 
															&miy, 
															&miz,
															&max, 
															&may, 
															&maz,
															dlg.m_extrusion_val * mu,
															dlg.m_ang_x,
															dlg.m_ang_y,
															dlg.m_ang_z,
															TRUE, 
															...
															dlg.m_matrix_dy * mu,
															dlg.m_matrix_dz * mu,
															(dlg.m_rnd_vtx?m_outline_poly.GetAt(i).GetW():0) );
			if( dlg.m_obj_type )
				nHoles++;
			///else if( dlg.m_extrusion_rot > BY_ZERO || dlg.m_extrusion_rot < -BY_ZERO )
			//{
			//	//skip 
			//}
			else
			...
			str.Format("module Poly%d_%s ()\n{\n%s", i, *fileName, ddSPACE );
			file.WriteString( str );

			// replace separator and save color data
			s->Replace( sep, "\n"+ddSPACE );
			s->Replace( "`", "\"" ); // (for color name)

			// save color data
			{
				int f = s->Find(dlg.CLR);
				if( f > 0 )
				{
					int fe = s->Find( ")", f );
					if( fe > 0 )
					{
						CString color = s->Mid( f, fe-f+1 );
						CString color_str_mem = color;
						CArray<CString> c_arr;
						CString key;
						int nstr = ParseKeyString( &color, &key, &c_arr );
						if( nstr >= 2 )
						{
							if( c_arr[0].Find(",") > 0 )
							{
								color.Replace("\"","");
								s->Replace( color_str_mem, color );
							}
						}
						if( dlg.m_obj_type == 0 ) // (if it not hole)
							color_str = color;
					}
				}
			}
			// generate width
			double W = m_outline_poly.GetAt(i).GetW()/mu;
			CString w;
			w.Format( "%.3f", W/2.0 );
			s->Replace( "*width*", w );

			// generate polygon points
			int ict = s->Find( "*content*" );
			int src = ict;
			while( src > 0 )
			{
				src--;
			...
			double cor1x = 0;
			double cor1y = 0;
			if( dlg.m_extrusion_scale > (1.0+BY_ZERO) || dlg.m_extrusion_scale < (1.0-BY_ZERO) )
			{
				RECT R = m_outline_poly.GetAt(i).GetCornerBounds(0);
				cor1x = (R.left+R.right)/2;
				cor1y = (R.top+R.bottom)/2;
				double x1 = m_outline_poly.GetAt(i).GetX(0);
				double y1 = m_outline_poly.GetAt(i).GetY(0);
				double mx = cor1x;
				double my = cor1y;
				{
					mx -= x1;
				...
				CString s1;
				s1.Format( "%s([ %.3f, %.3f", dlg.TRANSLATE, mx/mu, my/mu );
				s->Replace( "*center1*", s1 );
				s1.Format( "%s([ %.3f, %.3f", dlg.TRANSLATE, x1/mu, y1/mu );
				s->Replace( "*corner1*", s1 );
			}
			else 
			{
				cor1x = m_outline_poly.GetAt(i).GetX(0);
				cor1y = m_outline_poly.GetAt(i).GetY(0);
				CString s1;
				s1.Format( "%s([ %.3f, %.3f", dlg.TRANSLATE, cor1x/mu, cor1y/mu );
				s->Replace( "*corner1*", s1 );
			}
			CString polyPts = m_outline_poly.GetAt(i).GetOpenscadPolyPts(	doc->m_units, 
																			...
																			cor1x, 
																			cor1y, 0 );
			s->Replace( "*content*", polyPts );
			file.WriteString( *s );
			file.WriteString( "// end of module\n}\n" );
		}
		//
		// создаем модуль по названию футпринта
		//
		file.WriteString( "\n\n// Footprint\n" );
		str.Format("module fp_%s ()\n{\n%s", *fileName, sp+ddSPACE );
		file.WriteString( str );
		sp += ddSPACE;
		//
		// если есть дыры то создаем команду diff
		//
		if( nHoles )
		{
		...
		//
		// затем создаем команду union
		//
		if( dlg.m_fill_space )
			str.Format("%s%s\n%s{\n%s", color_str, dlg.HULL, sp, sp+ddSPACE );
		else 
			str.Format("union()\n%s{\n%s", sp, sp+ddSPACE );
		file.WriteString( str );
		sp += ddSPACE;
		//
		// пишем объекты, не дыры,
		// (объединяем в union)
		int cnt = 0;
		for( int i=0; i<t_OpenScadData.GetSize(); i++ )
		{
			CString * s = &t_OpenScadData.GetAt(i);
			if( s->GetLength() == 0 )
				continue;
			if( s->Find( dlg.ISHOLE ) > 0 )
				continue;
			int m = s->Find( dlg.MATRIX );
			if( m > 0 )
			{
				int em = s->Find( sep, m );
				CString smatrix = s->Mid( m, em-m );
				CString key;
				CArray<CString> arr;
				int np = ParseKeyString( &smatrix, &key, &arr );
				if( np>= 7 )
				{
					int npx = my_atoi( &arr[0] );
					int npy = my_atoi( &arr[1] );
					int npz = my_atoi( &arr[2] );
					double dx = my_atof( &arr[3] );
					double dy = my_atof( &arr[4] );
					double dz = my_atof( &arr[5] );
					for( int xi=0; xi<npx; xi++ )
						for( int yi=0; yi<npy; yi++ )
							for( int zi=0; zi<npz; zi++ )
							{
								cnt++;
								if( xi || yi || zi )
									str.Format("%s([ %.3f, %.3f, %.3f ]) {Poly%d_%s();}\n%s", dlg.TRANSLATE, dx*(double)xi, dy*(double)yi, dz*(double)zi, i, *fileName, sp );
								else
									str.Format("Poly%d_%s();\n%s", i, *fileName, sp );
								file.WriteString( str );
							}
				}
			}
		...
		// закрываем diff если был создан
		//
		if( nHoles )
		{
			sp.Truncate( sp.GetLength()-ddSPACE.GetLength() );
		...
		//
		sp.Truncate( sp.GetLength()-ddSPACE.GetLength() );
		str.Format("// end of module\n%s", sp );
		file.WriteString( str );
		str.Format("}\n%s", sp );
		file.WriteString( str );
		//
		// создаем модуль с глобальными параметрами
		//
		double h = 100000.0 / mu;
		if( minx == INT_MAX )
		{
			//if( nHoles > 0 )?
				//AfxMessageBox( "No objects, but only holes ("+*fileName+")", MB_ICONERROR );?
			minx = miny = minz = maxx = maxy = maxz = 0;
		}
		
		
		int intCx = (minx + maxx)/2;
		int intCy = (miny + maxy)/2;
		int intCz = minz;
		int gdx = intCx;
		int gdy = intCy;
		int gdz = intCz;
		double ddz = (double)gdz;
		ddz -= my_atof( &dlg.s_global_z );
		double ddx = (double)gdx/mu;
		double ddy = (double)gdy/mu;
		ddz = ddz/mu;
		ddz -= h;
		file.WriteString( "\n\n// Footprint\n" );
		str.Format("module F%s ()\n{\n%s", *fileName, sp+ddSPACE );
		file.WriteString( str );
		sp += ddSPACE;
		str.Format("difference()\n%s{\n%s", sp, sp+ddSPACE );
		file.WriteString(str);
		...
		//m_fp.m_centroid_x
		if( m_centroid_x || m_centroid_y )
		{
			double centroidx = m_centroid_x / mu;
			double centroidy = m_centroid_y / mu;
			str.Format( "%s([ %.3f,  %.3f,  0.0 ])\n%s    ", dlg.TRANSLATE, centroidx, centroidy, sp );
			file.WriteString( str );
		}
		double gl_r = my_atof( &global_r );
		if( abs(gl_r) > BY_ZERO )
		{
			str.Format( "%s[ 0.0,  0.0,  %s ])\n%s    ", dlg.ROTATE, global_r, sp );
			file.WriteString( str );
		}
		if( abs(ddx) > BY_ZERO || abs(ddy) > BY_ZERO || abs(ddz) > BY_ZERO )
		{
			str.Format( "%s([ %.3f,  %.3f,  %.3f ])\n%s    ", dlg.TRANSLATE, -ddx, -ddy, -ddz, sp );
			file.WriteString( str );
		}
		str.Format("fp_%s();\n\n%s", *fileName, sp );
		file.WriteString( str );
		...
		if( GetNumPins() )
		{
			...
			for( int i=0; i<GetNumPins(); i++ )
			{
				...
				if( ps->top.shape != PAD_NONE )
					padPresent = TRUE;
				if( ps->bottom.shape != PAD_NONE )
					padPresent = TRUE;
				if( ps->hole_size && padPresent )
				{
					hPresent = TRUE;
					break;
				}
			...
			for( int stp=0; stp<2; stp++ )
			{
				if( stp && hPresent )
				{
					// на втором шаге закрываем union:
					sp.Truncate( sp.GetLength()-ddSPACE.GetLength() );
					str.Format("// end of union\n%s", sp );
					file.WriteString( str );
					str.Format("}\n%s", sp );
				...
				for( int i=0; i<GetNumPins(); i++ )
				{
					...
					if( ps->bottom.shape != PAD_NONE )
						padPresent = TRUE;
					double psx = ps->x_rel / mu;
					double psy = ps->y_rel / mu;
					CString bh;
					bh.Format( "board_h+%.3f", h/2 );
					CString bhh;
					bhh.Format( "board_h+%.3f", h );
					double hsz = ps->hole_size / mu;
					...
					if( stp== 0 ) while( p )
					{
						if( p->shape == PAD_ROUND || p->shape == PAD_OCTAGON )
						{
							double szx = p->size_Y / mu;
							str.Format("%s([ %.3f, %.3f, %s ])\n  %s%s %.3f )\n    %s%s( %.3f )\n      %scircle( d=%.3f );\n%s", dlg.TRANSLATE, psx, psy, ph, sp, dlg.ROTATE, ang, sp, dlg.L_EXTRUDE, h, sp, szx, sp );
							file.WriteString( str );
						}
						else if( p->shape != PAD_NONE )
						{
							str.Format("%s([ %.3f, %.3f, %s ])\n%s  %s( %.3f )\n%s    polygon(", dlg.TRANSLATE, psx, psy, ph, sp, dlg.L_EXTRUDE, h, sp );
							file.WriteString( str );
							CPoint PTS[12];
							if( p->shape == PAD_SQUARE )
							{
							...
							int npts = Gen_RndRectPoly( 0, 0, (p->shape==PAD_SQUARE?p->size_Y:p->size_X), p->size_Y, p->radius, ps->angle, PTS, 12 );
							{
								for( int ipts=0;ipts<npts; ipts++ )
								{
									double fp1 = PTS[ipts].x / mu;
									double fp2 = PTS[ipts].y / mu;
									if( ipts == 0 )
										file.WriteString( "[" );
									if( ipts < npts-1 )
										str.Format("[ %.3f, %.3f ],\n%s            ", fp1, fp2, sp );
			...
			sp.Truncate( sp.GetLength()-ddSPACE.GetLength() );
			str.Format("// end of pins\n%s", sp );
			file.WriteString( str );
			str.Format("}\n%s", sp );
			file.WriteString( str );
		}
		//
		// добавляем доп. код BODY, если есть
		//
		...
		file.WriteString(str);
		//
		// добавляем доп. код HOLE, если есть
		//

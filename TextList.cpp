...
void CText::Draw( CDisplayList * dlist, SMFontUtil * smfontutil, CArray<CPoint> * m_stroke )
{
	...
	if( smfontutil )
	{
		...
		double letter_spacing = sp_scale*3.5*smfontutil->m_letter_spacing;
		//
		double xc = 0.0;
		int l_map=0;
		...
		for( int ic=0; ic<m_nchars; ic++ )
		{
			...
			int nstrokes;
			nstrokes = smfontutil->GetCharStrokes( m_str[ic], smfontutil->m_font_number, &min_x, &min_y, &max_x, &max_y,
					coord, 64 );
			if( nstrokes > 0 )
			{
...
int CTextList::WriteTexts( CStdioFile * file )
{
	...
	try
	{
		...
		{
			t = text_ptr[it];
			line.Format( "text: \"%s\" %d %d %d %d %d %d %d %d %d %d\n", t->m_str,
				t->m_x, t->m_y, t->m_layer, t->m_angle, t->m_mirror,
				t->m_font_size, t->m_stroke_width, t->m_bNegative, t->m_merge, t->m_submerge );
	...
	while( 1 )
	{
		...
		else if( in_str.Left(5) == "text:" )
		{
			...
			if( str.GetLength() )
			{
				...
				int s_merge = -1;
				if ( np >= 12 )
					s_merge = my_atoi( &p[10] );
				if( font_size < NM_PER_MIL )
					font_size = NM_PER_MM;
...
void CTextList::TextUndoCallback( int type, void * ptr, BOOL undo )
{
	...
	if( undo )
	{
		...
		else if( type == CTextList::UNDO_TEXT_DELETE )
		{
			...
			new_text->m_merge = un_t->u_merge;
			new_text->m_submerge = un_t->u_submerge;
		}
	}
	delete un_t;
}

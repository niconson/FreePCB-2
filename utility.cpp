...
{
	if( fp_lay == LAY_FP_NOTES_T )
		return LAY_REFINE_TOP;
	else if ( fp_lay == LAY_FP_NOTES_B )
		...
		return LAY_BOTTOM_COPPER+1;
	else if( fp_lay == LAY_FP_PAD_THRU )
		return LAY_PAD_THRU;
	else
...
{
	if( pcb_lay == LAY_REFINE_TOP )
		return LAY_FP_NOTES_T;
	else if ( pcb_lay == LAY_REFINE_BOT )
		return LAY_FP_NOTES_B;
	else if( pcb_lay == LAY_SILK_TOP )
		return LAY_FP_SILK_TOP;
	else if( pcb_lay == LAY_SILK_BOTTOM )
		return LAY_FP_SILK_BOTTOM;
	else if( pcb_lay == LAY_TOP_COPPER )
		return LAY_FP_TOP_COPPER;
	else if( pcb_lay == LAY_BOTTOM_COPPER )
		return LAY_FP_BOTTOM_COPPER;
	else if( pcb_lay > LAY_BOTTOM_COPPER )
		return LAY_FP_INNER_COPPER;
	else
		return LAY_FP_SELECTION;
}

int Get_Side ( int lay )
{
	if( lay == LAY_REFINE_TOP || 
		lay == LAY_SILK_TOP ||
		lay == LAY_TOP_COPPER )
		return 0;
...
{
	if( fp_lay == LAY_FP_TOP_PASTE || 
		fp_lay == LAY_FP_TOP_MASK ||
		fp_lay == LAY_FP_SILK_TOP ||
		fp_lay == LAY_FP_TOP_COPPER ||
		fp_lay == LAY_FP_NOTES_T )
		return 0;
	else if( fp_lay == LAY_FP_BOTTOM_PASTE || 
		fp_lay == LAY_FP_BOTTOM_MASK ||
		fp_lay == LAY_FP_SILK_BOTTOM ||
		fp_lay == LAY_FP_BOTTOM_COPPER ||
		fp_lay == LAY_FP_NOTES_B )
		return 1;
	else
		return 2;
}
		...
		if( i_start == -1 )
			if( abs(dang) < 44.0 )
			{
				if( !Check_45(v,nv) && !Check_90(v,nv) ) 
				{
					...
					else if( abs( d1-d2 ) < _2540 )
					{
						i_start = ii-1;
						//ang_start = dang;
						layer_start = c->seg[ii-1].layer;
					}
				}
			}
...
int RemoveColinearSegments( CArray<CPolyLine> * arr )
{
	...
	for( int i=0; i<arr->GetSize(); i++ )
	{
		...
		for( int ii=1; ii<(cl?arr->GetAt(i).GetNumCorners():arr->GetAt(i).GetNumSides()); ii++ )
		{
			int iprev = arr->GetAt(i).GetIndexCornerBack(ii);
			int inext = arr->GetAt(i).GetIndexCornerNext(ii);
...
int Gen_RndRectPoly (int x, int y, int dx, int dy, float rad, float ang, CPoint * OutPut, int npoints, float th_w )
{
	if( npoints%4 )
		return 0;
	int npo = 0;
	...
	else
	{
		if( npoints < 20 )
			return 0;
		OutPut[npo].x = + (float)dx/2.0;
		OutPut[npo].y = + (float)th_w/2.0;
		npo++;
		OutPut[npo].x = + (float)dx/2.0;
		OutPut[npo].y = + (float)dy/2.0 - (float)rad;
		npo++;
		OutPut[npo].x = + (float)dx/2.0 - (float)rad;
		OutPut[npo].y = + (float)dy/2.0;
		npo++;
		OutPut[npo].x = + (float)th_w/2.0;
		OutPut[npo].y = + (float)dy/2.0;
		npo++;
		OutPut[npo].x = + (float)th_w/2.0;
		OutPut[npo].y = + (float)th_w/2.0;
								   ...
								   int max_cl, int * x, int * y )
{
	...
	if( x1i == x1f )
		style1 = CPolyLine::STRAIGHT;
	else if( y1i == y1f )
		style1 = CPolyLine::STRAIGHT;
	if( x2i == x2f )
		style2 = CPolyLine::STRAIGHT;
	else if( y2i == y2f )
		style2 = CPolyLine::STRAIGHT;
...
CSize GetTextHeight( SMFontUtil * smfontutil )
{
	...
	int nLines = smfontutil->GetCharStrokes( 'I', (FONT_TYPE)smfontutil->m_font_number, &min_x, &min_y, &max_x, &max_y, coord, 64 );
	if( nLines )
	{
		CSize H;
		H.cx = min_y;
